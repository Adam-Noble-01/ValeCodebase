// =============================================================================
// VALEVISION3D - LAYOUT EDITOR - WEB VIEWER
// =============================================================================
//
...
//
// PORT NOTE:
// - Ported from   : TrueVision3D 51__System__LayoutEditor/80__Feature__WebViewer/Na__LayoutEditor__WebViewer__.js
// - Ported on     : 18-Sep-2026 for ValeVision3D v2.65.0
// - Parity        : adapted
// - Divergences   : ValeVision has no site plan sheets, so Documents() is the
//                   sheet order as the model gives it, then the specification.
...
// -----------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    // The sheets in the order the model gives them, then the specification -
    // the same order the tab strip draws, so anyone who has seen the editor
    // finds the documents where they left them. TrueVision splits site plan
    // sheets out and puts them after the plus; ValeVision has no drawing type
    // on a sheet, so there is one run of sheets and nothing to split.
    // ------------------------------------------------------------
    function Na__LeVw__Documents() {
        const list = Na__LeModel__GetSheets().map((sheet) => ({ id : sheet.Sheet__Id, name : sheet.Sheet__Name, kind : 'drawing', sheet : sheet }));
