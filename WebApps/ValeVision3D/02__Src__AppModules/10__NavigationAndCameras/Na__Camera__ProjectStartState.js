// =============================================================================
// VALEVISION3D - CAMERA PROJECT START STATE
// =============================================================================
//
...
//   sequence immediately after the project.json camera config and orbit
//   target have been applied.
// - Holds BOTH the raw project.json Camera__DefaultPosition block (the
//   authoritative source) AND an applied snapshot of the camera/controls
//   (the guaranteed-valid fallback when no project config exists).
...
// - Initial implementation for the floating navigation toolbar Reset View.
//
// =============================================================================
...
// -----------------------------------------------------------------------------
    ...
    let Na__CameraStartState__ProjectConfig = null;   // <-- Raw project.json Camera__DefaultPosition block (authoritative)
...
// -----------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    // Call AFTER the project.json camera config + orbit target have been
    // applied so the snapshot equals the project's true starting view.
    // projectCameraConfig may be null (no project.json) — the snapshot
    // alone then serves as the canonical reset state.
    // ------------------------------------------------------------
    function Na__CameraStartState__CaptureStartState(camera, controls, projectCameraConfig) {
        ...
        Na__CameraStartState__ProjectConfig = projectCameraConfig || null;   // <-- Raw project.json block (may be null)
        ...
        console.log('[ValeVision3D] Camera start state captured for Reset View.');
    }
    // ------------------------------------------------------------
...
// -----------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    // Restores the orbit camera to the canonical state loaded from the
    // project JSON. Returns true when a reset was performed. Callers must
    // already be in (or returning to) Orbit mode.
    // ------------------------------------------------------------
    function Na__CameraStartState__ResetView() {
        ...
        if (!camera || !controls || !snapshot) {
            console.warn('[ValeVision3D] Reset View skipped — start state not captured yet.');
            return false;
        }
        ...
        if (Na__CameraStartState__ProjectConfig) {
            ...
            Na__UiFeature__ApplyCameraConfig(camera, controls, configWithoutLegacyTarget);  // <-- Re-apply authoritative project.json values
        }
