// =============================================================================
// VALEVISION3D - PROJECTED LINEWORK - PIPELINE
// =============================================================================
//
...
// -----------------------------------------------------------------------------
    ...
    function Na__PlPipe__LogCost(definition, report) {
        ...
        console.log(
            '[ValeVision3D ProjectedLinework] ' + definition.ViewKey + ' [' + report.Backend + '] | ' +
    ...
    async function Na__PlPipe__RenderCurrent() {
        ...
        try {
            ...
            if (error && error.name === 'TooLargeError') {
                console.warn('[ValeVision3D ProjectedLinework] ' + error.message);
                Na__PlPipe__Dispatch(Na__PlPipe__STATUS_TOOLARGE, definition, { message : error.message });
                return false;
            }
            console.error('[ValeVision3D ProjectedLinework] Render failed:', error);
        ...
        void (async () => {
            ...
            if (!Na__PlPipe__Pending.has(cacheKey)) {
                ...
                try {
                    ...
                    console.warn('[ValeVision3D ProjectedLinework] Stored linework unavailable:', loadError);
                }
            ...
            console.error('[ValeVision3D ProjectedLinework] Evaluate failed:', evaluateError);
        });
    }
    // ------------------------------------------------------------
...
// -----------------------------------------------------------------------------
    ...
    function Na__PlPipe__ClearCache() {
        ...
        console.log('[ValeVision3D ProjectedLinework] Caches cleared.');
    }
    // ------------------------------------------------------------
    ...
    function Na__PlPipe__Initialize(context) {
        ...
        void Na__ProjectedLinework__WebGpuBackend__ProbeHardware().then((probe) => {
            console.log('[ValeVision3D ProjectedLinework] Graphics probe: '
        ...
        void Na__PlCfg__Ready().then(() => {
            console.log('[ValeVision3D ProjectedLinework] Loaded, enabled: ' + Na__PlCfg__IsEnabled() + '.');
            Na__PlPipe__Evaluate();
        });
        return true;
    }
