// =============================================================================
// VALEVISION3D - PROJECTED LINEWORK - WORKER POOL
// =============================================================================
//
...
//
// PORT NOTE:
// - Ported from   : Lantern Designer 02__Src__AppModules/27__System__ProjectedEdges2d/VghLantern__ProjectedEdges__WorkerPool__.mjs
...
// - Parity        : adapted
// - Divergences   :
//   - Run resolves { Segments, HiddenSegments, Owners, HiddenOwners } instead of a bare Float32Array; worker URL points at the .js worker.
...
// - Run resolves { Segments, HiddenSegments, Owners, HiddenOwners }. A shard's
//   owners are gathered in the same step as its segments and joined in the same
//   order (JoinOwners), so index k of the owners still describes segment k
//   whatever order the shards finish in. Ported from TrueVision3D (Edge Styles).
//
        ...
        try {
            ...
            while (Na__ProjectedLinework__WorkerPool__Workers.length < wanted) {
                ...
                worker.onerror  =  function(errorEvent) {
                    console.error('[ValeVision3D ProjectedLinework] Clip worker failed, falling back to the main thread:', errorEvent.message || errorEvent);
            ...
            console.warn('[ValeVision3D ProjectedLinework] Clip workers unavailable, using the main thread:', createError);
            Na__ProjectedLinework__WorkerPool__Disabled  =  true;
...
// -----------------------------------------------------------------------------
    ...
    // options.IncludeHiddenEdges was set (ValeVision addition: the hidden line
    ...
    export async function Na__ProjectedLinework__WorkerPool__Run(soup, edges, options, settings) {
        ...
        try {
            ...
            console.warn('[ValeVision3D ProjectedLinework] Pooled clip failed, retrying on the main thread:', poolError);
            Na__ProjectedLinework__WorkerPool__Disabled  =  true;
            Na__ProjectedLinework__WorkerPool__Dispose();
