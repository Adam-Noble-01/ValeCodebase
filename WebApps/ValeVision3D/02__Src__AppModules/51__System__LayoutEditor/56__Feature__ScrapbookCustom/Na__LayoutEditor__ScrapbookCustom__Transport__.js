// =============================================================================
// VALEVISION3D - LAYOUT EDITOR - CUSTOM SCRAPBOOK - TRANSPORT
// =============================================================================
//
...
// PURPOSE    : Read the Custom Scrapbook's index and item files, and save and delete items through the local development server
// CREATED    : 19-Sep-2026
//
// DESCRIPTION:
...
// - READING WORKS EVERYWHERE. On the local development server - Whitecardopedia's
//   server.py, which serves this app too - the index is asked of the API,
//   which rebuilds it from the folders first, so a file dropped in by hand is
//   there. Anywhere else - a static server, the live website - the index file
//   itself is read, which is as fresh as the last save that was pushed. Item
//   files are plain GETs in both cases.
...
//   that answers /api/check-localhost but not the scrapbook routes was started
//   before they existed, and that is reported as a restart, not as "no
//   server".
// - THAT SERVER ANSWERS AN UNKNOWN GET WITH A PAGE OF HTML AND 200, so an
//   answer counts only when it parses as the JSON that was expected.
...
// - Server side: WebApps/Whitecardopedia/Server__ValeVisionScrapbook__Api__.py,
//   registered by the server.py beside it.
//
// -----------------------------------------------------------------------------
//
// PORT NOTE:
// - Ported from   : TrueVision3D 56__Feature__ScrapbookCustom/Na__LayoutEditor__ScrapbookCustom__Transport__.js,
//                   its 1.0.0 (TrueVision v2.85.0)
// - Ported on     : 20-Sep-2026 for ValeVision3D v2.69.0
// - Parity        : adapted
// - Divergences   : THE SERVER IS A DIFFERENT ONE. TrueVision saves through the ProjectVision
//                   local server and knows it by the service name in /api/health. This app is
//                   served by Whitecardopedia's server.py, which has no /api/health: it is known
//                   by /api/check-localhost answering { isLocalhost : true }. The routes are
//                   /api/valevision/scrapbook..., in Server__ValeVisionScrapbook__Api__.py beside
//                   server.py. That server answers an unknown GET with a page of HTML and 200,
//                   which is why every answer here must parse as the JSON expected before it
//                   counts - TrueVision's code already did that, so reading is unchanged.
...
// -----------------------------------------------------------------------------
    ...
    // MODULE CONSTANTS | The App Root, the Server's Probe and Where an Index Came From
    // ------------------------------------------------------------
    const Na__LeScrapCustomIo__AppRootUrl    = new URL('../../../', import.meta.url);      // <-- 56__Feature / 51__System / 02__Src: the folder Index.html is in
    const Na__LeScrapCustomIo__ServerProbe   = '/api/check-localhost';                      // <-- Only the local development server answers this, with { isLocalhost : true }
    ...
    // ------------------------------------------------------------
    let Na__LeScrapCustomIo__Place = { contentFolder : '51__LayoutEditor__UserScrapbookContent', indexFile : 'UserScrapbook__Index__.json', apiPath : '/api/valevision/scrapbook' };
    // ------------------------------------------------------------
...
// -----------------------------------------------------------------------------
    ...
    // HELPER FUNCTION | Is the Local Development Server the One Answering
    // ------------------------------------------------------------
    ...
    // all. A static server answers the probe 404; the development server
    // answers it in JSON. Never throws.
    // ------------------------------------------------------------
    async function Na__LeScrapCustomIo__IsLocalDevServer() {
        try {
            const response = await fetch(window.location.origin + Na__LeScrapCustomIo__ServerProbe, { cache : 'no-store' });
            const answer   = response.ok ? await response.json().catch(() => null) : null;
            return !!(answer && answer.isLocalhost === true);
        } catch (error) {
            return false;
        }
    }
...
// -----------------------------------------------------------------------------
    ...
    //     why       when not writable: 'restart' for a development server
    ...
    async function Na__LeScrapCustomIo__ReadIndex() {
        ...
        if (Na__AppUtils__IsRunningOnLocalhost()) {
            try {
                ...
                if (await Na__LeScrapCustomIo__IsLocalDevServer()) why = Na__LeScrapCustomIo__WHY_RESTART;
...
// -----------------------------------------------------------------------------
    ...
    // error of its own; a static server does not, and a development server
    // without the routes answers 405.
    // ------------------------------------------------------------
    async function Na__LeScrapCustomIo__Post(suffix, payload) {
        try {
            ...
            if (await Na__LeScrapCustomIo__IsLocalDevServer()) {
                return { ok : false, answer : null, error : 'the local development server is running without the scrapbook routes (' + response.status + ') - restart it' };
            }
            return { ok : false, answer : null, error : 'no local save server at ' + window.location.origin + ' (' + response.status + ') - serve the app with Whitecardopedia\'s start_server' };
        } catch (error) {
