// =============================================================================
// VALEVISION3D - PLAN DIMENSIONS - CONFIG STATE
// =============================================================================
//
...
//
// PORT NOTE:
// - Ported from   : TrueVision3D 02__Src__AppModules/44__System__PlanDimensions/Na__PlanDimensions__Data__.js (config regions)
// - Source version: 1.0.0 (31-Aug-2026)
// - Ported on     : 09-Sep-2026 for ValeVision3D v2.18.0 (port Phase 2)
// - Parity        : adapted (split)
// - Divergences   :
//   - Extracted from the TrueVision data module; the getters are byte-for-byte the same, only their home changed.
    ...
    async function Na__PlanDim__Load() {
        try {
            ...
            if (response.ok) {
                ...
                console.warn(`[ValeVision3D] Plan dimensions config HTTP ${response.status}; using fallbacks.`);
            }
        } catch (error) {
            console.warn('[ValeVision3D] Plan dimensions config load failed; using fallbacks.', error);
        }
