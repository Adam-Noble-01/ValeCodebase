...
//           North__SetIso                           when
//       }
//   and not in a top-level key of its own: what north is FOR is drawings, the
//   block's own save (Na__DrawData__Save - R2 first, then the local mirror)
//   already carries whatever the block holds, and the same key in the same
//   place keeps a project readable by TrueVision and ValeVision alike.
...
//
// PORT NOTE:
// - Ported from   : TrueVision3D 46__System__NorthDirection/Na__North__ProjectJson__Data__.js,
//                   its 1.0.0 (TrueVision v2.85.0)
// - Ported on     : 20-Sep-2026 for ValeVision3D v2.67.0
// - Parity        : adapted
// - Divergences   : The key, its fields and every reader are TrueVision's, so a project reads the
//                   same in either app. Only the description of the save differs: here the block
//                   rides Na__DrawData__Save into project.json, R2 first and then the Flask mirror.
...
// -----------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    import { Na__DrawData__GetBlock, Na__DrawData__Save } from '../42__System__DrawingViewCore/Na__DrawView__ProjectData__.js';
...
// -----------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    // R2 first, then the local Flask mirror. Everything else in the block goes
    // with it, as it does for Save Sheets and Save Elevations.
    // ------------------------------------------------------------
    function Na__NorthData__Save(showToast) {
