...
//   sheet: the project's Layout Mode switch is on
//   (LayoutEditor__DrawingsData__LayoutModeEnabled, set from the Dev menu and
//   off by default) AND the project has at least one sheet, on localhost and
...
//   dock at the bottom of the screen and which of its two reading surfaces is
//   showing. Not building the authoring surface is what keeps it off a
//   reader's paper. The tab strip is unchanged and still does the navigating.
...
//
// INTEGRATION:
// - Imported and initialized by Na__LayoutEditor__Loader__, with the render
//   context, the first time the editor is needed; never at start-up. The
//   tab strip and Dev menu call Enter and Leave through the loader.
//
// -----------------------------------------------------------------------------
//
// PORT NOTE:
...
//   row, and AttachSheetInput binds nothing, so the editing tools are never
//   attached rather than attached and told to behave. Entering a sheet and
//   opening the specification route through Na__LayoutEditor__WebViewer__,
//   which owns the dock and which reading surface is on. Leaving stands both
//   surfaces down.
//
// 17-Sep-2026 - Version 1.16.0
...
//
// 15-Sep-2026 - Version 1.15.1
// - A scene broadcast (a scene or drawing card added, renamed, regrouped or
//   removed) refreshes the Viewport panel while a sheet is open, so its Scene
//   lists follow without leaving the sheet.
//
// 15-Sep-2026 - Version 1.15.0
// - IsAvailable is one rule everywhere: Layout Mode on AND at least one
//   sheet. The live site used to ignore the switch, and localhost used to
//   offer the editor with no sheets.
// - No longer imported by index.html. Na__LayoutEditor__Loader__ imports and
//   initialises this module the first time the editor is needed, then
//   announces the project load the editor started too late to hear.
//
...
//   stage) when the shell is built. SpecEd mounts over the host.
...
// -----------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    // ONE-WAY, DELIBERATELY. The viewer never imports this module: the two
    // things it needs to do - open a sheet, open the specification - are handed
    // to it as callbacks when its chrome is built, so the pair can never form
    // an import cycle. The tab strip loads before the editor and never touches
    // the viewer at all, which is what keeps it off the start-up path.
...
// -----------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    // One rule, on localhost and on the live site alike: the project's Layout
    // Mode switch is on AND it has at least one sheet. A project nobody has
    // laid out carries no tabs, and switching Layout Mode off takes the
    // drawings away from clients too. Na__LayoutEditor__Loader__ answers the
    // same question from the raw block before this module has loaded, so the
    // two must stay one rule.
    // ------------------------------------------------------------
    function Na__LeMode__IsAvailable() {
        if (!Na__LeCfg__IsEnabled()) return false;
        return Na__DrawData__GetLayoutModeEnabled() && Na__LeModel__GetSheets().length > 0;
    }
    // ------------------------------------------------------------
    ...
    function Na__LeMode__Dispatch() {
        ...
        Na__LeVw__Sync();                                                      // <-- The viewer's dock follows the same announcement the tab strip does
...
// -----------------------------------------------------------------------------
    ...
    function Na__LeMode__Initialize(context) {
        ...
        Na__LeMode__ReadyOnce = Promise.all([ Na__LeCfg__Ready(), Na__LeEdge__Ready(), Na__LeComposite__Ready(), Na__LeGrad__Ready(), Na__LeDash__Ready(), Na__DrawCfg__Load() ]).then(() => {   // <-- None of the six rejects, so a missing file cannot hold the editor back
            ...
            window.addEventListener('na-presentation-mode-scenes-loaded',  () => { if (Na__LeMode__Active) Na__LePanels__Refresh('viewport'); });   // <-- A scene or drawing card added, renamed or regrouped: the Scene lists follow
            window.addEventListener('na-presentation-mode-scenes-cleared', () => { if (Na__LeMode__Active) Na__LePanels__Refresh('viewport'); });   // <-- The last scene removed
            console.log('[ValeVision3D] Layout Editor ready (' + (Na__LeMode__IsEditable() ? 'editable' : (Na__LeVw__IsViewerMode() ? 'web document viewer, read-only' : Na__LeCfg__GetLabel('ReadOnlyNote', 'read-only'))) + ').');
            return true;
        }).catch((error) => {
