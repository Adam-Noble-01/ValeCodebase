...
$Na__AppUrl = "http://127.0.0.1:8002/ValeSpec__App__.html"
...
function Na__Starter__TestHealth {
    try {
        $response = Invoke-WebRequest -UseBasicParsing -Uri "http://127.0.0.1:8002/api/system/health" -TimeoutSec 2
...
function Na__Starter__GetListenerConnection {
    return Get-NetTCPConnection -LocalPort 8002 -State Listen -ErrorAction SilentlyContinue | Select-Object -First 1
}

Write-Host "=============================================================================" -ForegroundColor Cyan
Write-Host " VALESPEC - LOCALHOST STARTER (POWERSHELL)" -ForegroundColor Cyan
Write-Host "=============================================================================" -ForegroundColor Cyan
Write-Host " Launching python ValeSpec__FlaskServer__Localhost__.py on $Na__AppUrl" -ForegroundColor Yellow
Write-Host "=============================================================================" -ForegroundColor Cyan
...
if ($PortConnection) {
    ...
    Write-Host ""
    Write-Host " [INFO] Port 8002 is already in use (PID $PortPid)." -ForegroundColor Yellow

    if (Na__Starter__TestHealth) {
        Write-Host " [INFO] ValeSpec health endpoint is responding." -ForegroundColor Green
        $choiceRaw = ""
        ...
        if ($choice -eq "O") {
            ...
            Na__Starter__PauseIfNeeded "ValeSpec app opened. Press Enter to close this window..."
            exit 0
        }
        ...
        Write-Host " [WARNING] Port 8002 is busy but ValeSpec health check failed." -ForegroundColor Yellow
...
if ($PythonExe) {
    ...
    $LauncherArgs = @("-u", "ValeSpec__FlaskServer__Localhost__.py", "--host", "127.0.0.1", "--port", "8002")
} elseif ($PyExe) {
    $LauncherPath = $PyExe.Source
    $LauncherArgs = @("-3", "-u", "ValeSpec__FlaskServer__Localhost__.py", "--host", "127.0.0.1", "--port", "8002")
} else {
...
if ($ExitCode -ne 0) {
    Write-Host ""
    Write-Host " [WARNING] ValeSpec server exited with code $ExitCode." -ForegroundColor Yellow
}

Na__Starter__PauseIfNeeded "ValeSpec server stopped. Press Enter to close this window..."
exit $ExitCode
