// =============================================================================
// VALEVISION3D - LAYOUT EDITOR - SHEET MODEL - VIEWPORTS
// =============================================================================
//
...
// - UpdateViewport's silent path sets the dirty flag through the State
//   unit's AssignDirty (an imported let cannot be assigned).
//
// INTEGRATION:
...
// - RegisterViewportNamer, ported from TrueVision3D's Viewports unit 1.1.0: one
//   function may offer a name for a viewport nobody has named, ahead of its
//   drawing's own. The viewport identity module uses it to call an elevation
//   viewport "North Elevation" from the project's north. A typed Viewport__Name
...
// -----------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    import { Na__FpData__GetPlanById } from '../../43__System__FloorPlanViews/Na__FloorPlan__ProjectJson__Data__.js';
    import { Na__ElevData__GetElevationById } from '../../46__System__ElevationViews/Na__Elevation__ProjectJson__Data__.js';
...
// -----------------------------------------------------------------------------
    ...
    function Na__LeModel__UpdateViewport(sheet, viewportId, patch, silent) {
        ...
        if (patch.modelLayers) {
            ...
            // time. The normaliser drops anything set back to true, so
        ...
        if (patch.compositeWeights) {
            // MERGED, ONE WEIGHT AT A TIME, and a null value CLEARS that weight
            // rather than storing it - which is how the panel's reset button
            // leaves no trace that anything was touched.
    ...
    function Na__LeModel__ResolveViewportSource(viewport) {
        ...
        const label = viewport.Viewport__Name
            || Na__LeModel__DerivedViewportName(viewport, { plan : plan, elevation : elevation, scene : scene })   // <-- An unnamed elevation viewport, named from the project's north
