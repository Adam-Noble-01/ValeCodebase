// =============================================================================
// VALEVISION3D - PRESENTATION MODE - DEV MENU SCENE GROUP EDITOR
// =============================================================================
//
...
//   AppConfig defaults the first time this panel renders - six groups with
...
//   project's stored data. On a project whose scenes were auto-built from
//   SketchUp camera data, the first group save materialises the explicit
//   scenes block exactly as Save All already does.
...
//
// PORT NOTE:
// - Ported from   : TrueVision3D 02__Src__AppModules/21__System__PresentationMode/Na__PresentationMode__DevMenu__GroupEditor__.js
// - Source version: 1.0.0 (31-Aug-2026)
// - Ported on     : 09-Sep-2026 for ValeVision3D v2.17.0
// - Parity        : adapted
// - Divergences   :
//   - Console prefix reads [ValeVision3D].
//   - The two window.confirm prompts use ValeVision's async Na__AppUtils__ConfirmDialog__Show,
//     so SetGroupEnabled and DeleteGroup are async and their row handlers await them.
// - Back-port     : the confirm-dialog form is the better one; carry it to TrueVision when its
//                   scene editor moves off window.confirm.
//
// -----------------------------------------------------------------------------
//
// DEVELOPMENT LOG:
// 09-Sep-2026 - Version 1.0.0
// - Ported from TrueVision3D for the Scene Groups feature (port Phase 1).
//
// =============================================================================
...
// -----------------------------------------------------------------------------
    ...
    // MODULE IMPORTS | Confirm Dialog (house convention for destructive dev actions)
    // ------------------------------------------------------------
...
// -----------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    async function Na__PmGroupDev__SetGroupEnabled(config, group, enabled) {
        ...
        if (sceneCount > 0) {
            ...
            const proceed = await Na__AppUtils__ConfirmDialog__Show({
                title        : 'Switch Group Off?',
                message      : message,
                confirmLabel : 'Switch Off',
                isDestructive: false
            });
            if (!proceed) return false;                                      // <-- Author backed out
    ...
    // ------------------------------------------------------------
    async function Na__PmGroupDev__DeleteGroup(config, group) {
        const labels  = Na__PresentationMode__SceneGroups__GetDevMenuLabels();
...
// -----------------------------------------------------------------------------
    ...
    function Na__PmGroupDev__BuildGroupRow(config, group, rowIndex, rowCount, onMutated) {
        ...
        enabledBox.addEventListener('change', async () => {
            const applied = await Na__PmGroupDev__SetGroupEnabled(config, group, enabledBox.checked);
            if (!applied) {
        ...
        deleteBtn.addEventListener('click', async () => {
            if (await Na__PmGroupDev__DeleteGroup(config, group)) onMutated();
        });
