...
//   composes safely with the model-toggle visibility systems via the THREE.js
//   visibility hierarchy.
// - MaxEngine-only optional feature; disabled by default in ValeVision config.
...
// -----------------------------------------------------------------------------
    ...
    // MODULE IMPORTS | Three.js and Unit Conversion
    // ------------------------------------------------------------
...
// -----------------------------------------------------------------------------
    ...
    // MODULE VARIABLES | Culling Configuration and Registry
    // ------------------------------------------------------------
...
// -----------------------------------------------------------------------------
    ...
    function Na__DistanceCulling__Initialize(config) {
        ...
        if (!config || config.RenderEffect__DistanceCulling__Enabled !== true) {
            ...
            console.log('[ValeVision3D] Distance culling disabled (config).');
            return;
        }
        ...
        console.log(
            `[ValeVision3D] Distance culling enabled: ${cullDistanceUnits}u (${cullDistanceMm}mm), ` +
...
// -----------------------------------------------------------------------------
    ...
    //   by each instance's instanceMatrix.
    ...
    function Na__DistanceCulling__RegisterRootChildren(rootGroup) {
        ...
        for (const itemNode of rootGroup.children) {
            ...
            // i.e. distanceSq > (cullDistance + radius)^2.
    ...
    function Na__DistanceCulling__RegisterModelGroups(loadedGroups) {
        ...
        console.log(
            `[ValeVision3D] Distance culling registered ${Na__DistanceCulling__Registry.length} item(s) ` +
