/* =============================================================================
   ...
   PURPOSE    : Build and Play - which class of interaction the pointer is doing
   CREATED    : 08-Aug-2026

   DESCRIPTION:
   - Two application modes, borrowed wholesale from the build / live split in a life
     simulation game, because it solves exactly the same problem.
   ...
   - Selection works in both, so the inspector is always reachable.

   ---------------------------------------------------------------------------

   WHY THIS EXISTS

   Without it the two gestures collide. A module's pad is its drag handle and its steps
   are its controls, and they sit a few centimetres apart in a space the user is
   orbiting around - so reaching for the pad and catching a step instead is not an
   occasional slip, it is the common case.
   ...
   amount of care from the user fixes it, because the two targets genuinely overlap.

   Modes make the collision impossible rather than unlikely. In Build mode a step
   cannot be clicked at all, so the pattern is safe by construction while the layout is
   being changed.
...
// -----------------------------------------------------------------------------
    ...
    // MODULE CONSTANTS | The Two Modes
    // ------------------------------------------------------------
    ...
    export const NaAudio__Mode__Both  =  Object.freeze([NaAudio__Mode.Build, NaAudio__Mode.Play]);
    // ------------------------------------------------------------


    // MODULE CONSTANTS | Presentation Per Mode
    // ------------------------------------------------------------
    // Read by the HUD indicator and by the ground stage. Held here rather than in the
    // HUD because the mode owns what it means, and two places will describe it.
    export const NaAudio__Mode__Presentation  =  Object.freeze({
        build : {
            Label       : 'Build',
            Pigment     : 'SlateBlue',
            Hint        : 'Move modules. Controls are frozen.',
            ShowGrid    : true
        },
        play : {
            Label       : 'Play',
    ...
    // ------------------------------------------------------------
    // Play is the default. The space arrives already arranged, and the first thing
    // anybody wants to do is hear it - not move it.
...
// -----------------------------------------------------------------------------
    ...
    // empty list means 'live in both', which keeps the common case free of ceremony.
...
// -----------------------------------------------------------------------------
    ...
    export function NaAudio__ModeManager__Set(mode) {
        if (mode !== NaAudio__Mode.Build && mode !== NaAudio__Mode.Play) {
            console.warn('[NaAudio ModeManager] Unknown mode "' + mode + '". Use NaAudio__Mode.Build or NaAudio__Mode.Play.');
            return currentMode;
        }
    ...
    // FUNCTION | Toggle Between Build and Play
    // ------------------------------------------------------------
    export function NaAudio__ModeManager__Toggle() {
        return NaAudio__ModeManager__Set(
            currentMode === NaAudio__Mode.Build ? NaAudio__Mode.Play : NaAudio__Mode.Build
        );
    }
    // ------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    // Tab toggles, because it is the one key nothing else in the application wants and
    // it reads as 'switch context'. preventDefault is essential - the browser's own
    // focus traversal would otherwise walk the transport bar on every press.
