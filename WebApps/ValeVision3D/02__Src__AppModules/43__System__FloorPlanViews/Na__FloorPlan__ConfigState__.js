...
// -----------------------------------------------------------------------------
    ...
    async function Na__FpCfg__Fetch() {
        try {
            ...
            if (!response.ok) {
                console.warn('[ValeVision3D] Floor plan config fetch failed (' + response.status + ') - using built-in defaults.');
                return false;
            }
            Na__FpCfg__Config = await response.json();
            return Na__FpCfg__IsEnabled();
        } catch (error) {
            console.warn('[ValeVision3D] Floor plan config unreadable - using built-in defaults.', error);
            return false;
        }
    }
    // ------------------------------------------------------------
