...
// -----------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    const PAPER_BUMP_EFFECT_ENABLED           = true;                        // <-- Enable bump effect
    const PAPER_BUMP_DEFAULT_STRENGTH         = 0.001;                       // <-- Overall displacement strength
    const PAPER_BUMP_STRENGTH_MIN             = 0.0;                         // <-- Minimum bump on white areas
    const PAPER_BUMP_STRENGTH_MAX             = 10.0;                         // <-- Maximum bump on colored areas
    const PAPER_BUMP_PAPER_SCALE              = 0.8;                         // <-- Paper texture tiling scale
    const PAPER_BUMP_SATURATION_MULTIPLIER    = 3.0;                         // <-- Saturation sensitivity
    const PAPER_BUMP_PARALLAX_STRENGTH        = 0.0001;                      // <-- Parallax offset strength
    const PAPER_BUMP_DISTANCE_STRENGTH_MIN    = 0.5;                         // <-- Minimum bump at far distance (50% effect)
    const PAPER_BUMP_DISTANCE_STRENGTH_MAX    = 4.0;                         // <-- Maximum bump at near distance (300% effect)
    const PAPER_BUMP_DISTANCE_FALLOFF         = 3.0;                         // <-- Distance falloff curve power (higher = sharper falloff)
    // ------------------------------------------------------------
...
// -----------------------------------------------------------------------------
    ...
    class PaperBumpEffect {
        constructor(scene, camera, paperTexturePath) {
            ...
            // ------------------------------------
            this.bumpStrength = PAPER_BUMP_DEFAULT_STRENGTH;                 // <-- Overall displacement strength
            this.bumpStrengthMin = PAPER_BUMP_STRENGTH_MIN;                  // <-- Minimum bump strength
            this.bumpStrengthMax = PAPER_BUMP_STRENGTH_MAX;                  // <-- Maximum bump strength
            this.paperScale = PAPER_BUMP_PAPER_SCALE;                        // <-- Paper tiling scale
            this.saturationMultiplier = PAPER_BUMP_SATURATION_MULTIPLIER;    // <-- Saturation sensitivity
            this.parallaxStrength = PAPER_BUMP_PARALLAX_STRENGTH;            // <-- Camera parallax strength
            this.distanceStrengthMin = PAPER_BUMP_DISTANCE_STRENGTH_MIN;     // <-- Minimum bump at far distance
            this.distanceStrengthMax = PAPER_BUMP_DISTANCE_STRENGTH_MAX;     // <-- Maximum bump at near distance
            this.distanceFalloffPower = PAPER_BUMP_DISTANCE_FALLOFF;         // <-- Distance falloff curve power
...
// -----------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    async function createPaperBumpEffect(scene, camera, paperTexturePath, enabled = null) {
        // #Region ------------------------------------------------
        // Effect Enabled Control
        // --------------------------------------------------------
        const effectEnabled = enabled !== null ? enabled : PAPER_BUMP_EFFECT_ENABLED;  // <-- Use passed enabled flag or default constant
        // #endregion ---------------------------------------------
...
// -----------------------------------------------------------------------------
    ...
    // Export createPaperBumpEffect to global scope for HTML access
    // ------------------------------------------------------------
    window.createPaperBumpEffect = createPaperBumpEffect;                     // <-- Export main function to global scope
    // ------------------------------------------------------------
