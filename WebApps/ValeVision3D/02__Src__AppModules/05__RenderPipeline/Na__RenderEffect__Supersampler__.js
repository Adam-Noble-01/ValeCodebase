// =============================================================================
// VALEVISION3D - RENDER PIPELINE - SUPERSAMPLER
// =============================================================================
//
...
// - This started as the video studio's own supersampler (11-Sep-2026). The
//   still exporter and the Layout Editor viewport bakes need exactly the same
//   maths, and a jitter table that exists twice is a jitter table that will
//   eventually disagree with itself, so it lives in the render pipeline now
//   and the video studio module is a re-export of this one.
//
// THE JITTER GOES INTO THE PROJECTION, NOT THE SCENE PASS:
// - The picture is not made of geometry, it is made of LINE WORK - the fat
//   outlines, the profile-line pass that finds silhouettes and creases, the
//   section cut outlines, fog and SSAO. Jittering only the scene render,
//   which is what three's SSAARenderPass does, would smooth the walls
//   beautifully and leave every line exactly as stepped as before. Since the
//   lines are the drawing, that is close to useless. Shifting the projection
//   at the root means every downstream pass inherits it. This is the single
//   most important decision in the module.
//
// SAMPLE PATTERNS:
...
// - COMPOSER ROUTE, which is every caller in ValeVision. The composer is told
//   to stop drawing to the canvas, so its last pass leaves the finished frame
//   in its read buffer, and that texture is handed to accumulate(). FXAA is
//   switched off by the caller for the duration: it would soften each sample
//   BEFORE the average, and averaging sixteen blurred pictures buys blur for
//   nothing. Present copies the total out raw, exactly as the FXAA pass it
//   stands in for would have.
// - TARGET ROUTE, which TrueVision needs and ValeVision does not call today.
//   A TrueVision drawing bypasses the composer entirely and draws to whatever
//   framebuffer is bound, so the caller asks for a sample target, binds it,
//   draws the whole frame into it, and accumulates that. COLOUR SPACE IS THE
//   TRAP: three applies the sRGB output transfer only when rendering to the
//   canvas, never to a render target, so a frame diverted into a target comes
//   out linear and a raw copy would wash it out. encodeSrgb makes the present
//   pass apply the transfer instead. Kept here rather than trimmed, so the
//   two apps hold one identical file.
//
...
//                   consumers; that generalisation comes back here as the shared module,
//                   and the video studio file now re-exports it.
//
// -----------------------------------------------------------------------------
//
// DEVELOPMENT LOG:
