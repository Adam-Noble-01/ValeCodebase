// =============================================================================
// VALEVISION3D - DRAWING VIEW CORE - CONFIG STATE
// =============================================================================
//
...
//
// PORT NOTE:
// - Ported from   : none (ValeVision original, following the TrueVision ConfigState pattern)
...
// -----------------------------------------------------------------------------
    ...
    async function Na__DrawCfg__Fetch() {
        try {
            ...
            if (!response.ok) {
                console.warn('[ValeVision3D] Drawing view config fetch failed (' + response.status + ') - using built-in defaults.');
                return false;
            }
            Na__DrawCfg__Config = await response.json();
            return true;
        } catch (error) {
            console.warn('[ValeVision3D] Drawing view config unreadable - using built-in defaults.', error);
            return false;
        }
    }
    // ------------------------------------------------------------
