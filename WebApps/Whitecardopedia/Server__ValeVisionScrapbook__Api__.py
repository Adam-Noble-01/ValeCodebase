#!/usr/bin/env python3
# =============================================================================
# VALEVISION3D - CUSTOM SCRAPBOOK API (FLASK BLUEPRINT)
# =============================================================================
#
# FILE       : Server__ValeVisionScrapbook__Api__.py
# MODULE     : ValeVisionScrapbookApi
# AUTHOR     : Adam Noble - Noble Architecture
# PURPOSE    : Save, list and delete the ValeVision Layout Editor's Custom Scrapbook items
# CREATED    : 20-Sep-2026
#
# DESCRIPTION:
# - The Custom Scrapbook is a folder of JSON files, one per item, in category
#   folders under:
#       WebApps/ValeVision3D/51__LayoutEditor__UserScrapbookContent/
#   A browser cannot write a file, so the app saves through these routes.
# - Registered by the server.py beside this file, which serves ValeVision3D
#   from its sibling folder. Localhost only; the public website has no such
#   server, and there the library is read-only.
...
#   made by hand, and a typo in a request must not grow the tree.
...
#   the scrapbook folder, where the index does not look, and stays on disk
#   until the folder is emptied by hand.
#
# ROUTES:
#   GET  /api/valevision/scrapbook                  the index, rebuilt first
#   POST /api/valevision/scrapbook/items            { category, name, item }
#   POST /api/valevision/scrapbook/items/delete     { file }  'Category/File.json'
#
# -----
#
# PORT NOTE:
# - Ported from   : the TrueVision repository's na-apps/ProjectVision__TrueVisionScrapbook__Api__.py, its 1.0.0
# - Ported on     : 20-Sep-2026 for ValeVision3D v2.69.0
# - Parity        : adapted - the folder it writes, the route prefix and the blueprint's name are this app's.
#                   Every rule about what may be written, every limit and the item document are the source's,
#                   so an item file saved by either app reads in the other.
#
# -----
#
# DEVELOPMENT LOG:
# 20-Sep-2026 - Version 1.0.0
# - Ported from the TrueVision Custom Scrapbook API's 1.0.0
#
# =============================================================================
...
VALEVISION_APP_DIR      = os.path.join(os.path.dirname(SCRIPT_DIR), 'ValeVision3D')  # <-- The sibling folder server.py serves as /ValeVision3D/
SCRAPBOOK_DIR           = os.path.join(VALEVISION_APP_DIR, '51__LayoutEditor__UserScrapbookContent')
...
valevision_scrapbook_api = Blueprint('valevision_scrapbook_api', __name__)
...
def _rebuild_index():
    ...
    index = {
        'UserScrapbook__Index__Meta': {
            ...
            'Meta__Description' : (
                'Every item in the ValeVision Layout Editor Custom Scrapbook. Written by the local development '
                'server (WebApps/Whitecardopedia/Server__ValeVisionScrapbook__Api__.py) from what is in the category '
...
@valevision_scrapbook_api.route('/api/valevision/scrapbook')
def scrapbook_index():
...
@valevision_scrapbook_api.route('/api/valevision/scrapbook/items', methods=['POST'])
...
@valevision_scrapbook_api.route('/api/valevision/scrapbook/items/delete', methods=['POST'])
