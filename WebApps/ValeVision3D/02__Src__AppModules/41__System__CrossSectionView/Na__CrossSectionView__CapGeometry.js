// =============================================================================
// VALEVISION3D - CROSS SECTION VIEW - CAP GEOMETRY ENGINE
// =============================================================================
//
// FILE       : Na__CrossSectionView__CapGeometry.js
// NAMESPACE  : Na__SectCap
// MODULE     : Cross Section View - Cap Geometry
...
//
// INTEGRATION:
// - Called by Na__CrossSectionView__SystemLogic on section insert, drag
//   (throttled) and drag end.
// - Returns plain typed arrays; the system logic owns mesh/material creation.
//
// -----------------------------------------------------------------------------
//
// DEVELOPMENT LOG:
...
// -----------------------------------------------------------------------------
    ...
    function Na__SectCap__CollectEligibleMeshes(root) {
        ...
        function visit(object) {
            ...
            if (object.userData && object.userData.naCrossSectionHelper) return; // <-- Never cut our own gizmos / caps
    ...
    function Na__SectCap__TriangulateLoops(loops, minLoopArea) {
        ...
        for (let i = 0; i < kept.length; i++) {
            ...
            try {
                ...
                console.warn('[CrossSection] Cap triangulation failed for one loop — outline kept, fill skipped.', triError);
            }
        }
