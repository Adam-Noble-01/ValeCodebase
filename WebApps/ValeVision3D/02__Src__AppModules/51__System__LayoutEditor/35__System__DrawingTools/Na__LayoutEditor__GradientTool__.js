// =============================================================================
// VALEVISION3D - LAYOUT EDITOR - GRADIENT TOOL
// =============================================================================
//
...
// - Na__LayoutEditor__ModeController__ waits on Ready alongside the editor's own config.
//
...
//
// PORT NOTE:
// - Ported from   : TrueVision3D 51__System__LayoutEditor/Na__LayoutEditor__GradientTool__.js
// - Ported on     : 13-Sep-2026 for ValeVision3D v2.26.0 (a TrueVision-first feature)
// - Parity        : verbatim
// - Divergences   : Header and console prefix only. The config JSON is verbatim and
//                   the record shape is shared, so either app reads the other's gradients.
        ...
        if (!Na__LeGrad__LoadPromise) {
            Na__LeGrad__LoadPromise = (async () => {
                try {
                    ...
                    if (!response.ok) {
                        console.warn('[ValeVision3D LayoutEditor] Gradient config fetch failed (' + response.status + ') - the built-in defaults will be used.');
                        return null;
                    }
                    Na__LeGrad__Config = await response.json();
                } catch (error) {
                    console.warn('[ValeVision3D LayoutEditor] Gradient config unavailable - the built-in defaults will be used.', error);
                }
