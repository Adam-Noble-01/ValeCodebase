// =============================================================================
// VALEVISION3D - PLAN ANNOTATIONS - DATA AND CONFIG
// =============================================================================
//
...
//
// PORT NOTE:
// - Ported from   : TrueVision3D 02__Src__AppModules/43__System__PlanAnnotations/Na__PlanAnnotations__Data__.js
        ...
        if (!Na__PlanAnno__LoadPromise) {
            Na__PlanAnno__LoadPromise = (async () => {
                try {
                    ...
                    if (!response.ok) {
                        console.warn('[ValeVision3D] Plan annotations config fetch failed (' + response.status + ') - using built-in defaults.');
                        return false;
                    }
                    Na__PlanAnno__Config = await response.json();
                    return Na__PlanAnno__IsEnabled();
                } catch (error) {
                    console.warn('[ValeVision3D] Plan annotations config unreadable - using built-in defaults.', error);
                    return false;
                }
            })();
        }
