...
// - The STYLES row carries the four toggles a Layout Editor viewport also has
//   (Projected Linework, Profile Linework, Glass Transparency Off, Whitecard),
//   so a carousel drawing and a sheet viewport of the same plan read the same.
//   The exclusion field takes comma-separated category tokens; blank means the
//   AppConfig default list.
...
// - Parity        : adapted
// - Divergences   :
//   - Styles row (four toggles, D33) and the exclusion tokens field (D19) added to the plan row,
//     built by the shared Na__DrawView__StyleRows__ so plans and elevations read the same.
// - Back-port     : the styles row, once TrueVision carries per-drawing styles.
//
// -----------------------------------------------------------------------------
//
// DEVELOPMENT LOG:
// 09-Sep-2026 - Version 1.2.0
// - Styles row and exclusion field moved to the shared Na__DrawView__StyleRows__
//   (port Phase 3); this module supplies the plan record accessors.
//
// 09-Sep-2026 - Version 1.1.0
// - Ported to ValeVision3D; styles row and exclusion field added.
//
...
// -----------------------------------------------------------------------------
    ...
    // MODULE IMPORTS | Floor Plan Config and Derived Values
    // ------------------------------------------------------------
...
// -----------------------------------------------------------------------------
    ...
    // MODULE CONSTANTS | Style Row Accessors (the plan record's own)
    // ------------------------------------------------------------
...
// -----------------------------------------------------------------------------
    ...
    //   onOffsetChange, onDepthChange, onStyleChange, onExclusionsChange,
    ...
    function Na__FpRow__BuildPlanRow(plan, handlers) {
        ...
        // STYLES AND EXCLUSIONS | Shared with the Layout Editor viewports
        rowRoot.appendChild(Na__DrawStyleRow__BuildStylesRow(plan, Na__FpRow__STYLE_ACCESSORS, handlers.onStyleChange));
        rowRoot.appendChild(Na__DrawStyleRow__BuildExclusionsRow(plan, Na__FpRow__STYLE_ACCESSORS, handlers.onExclusionsChange));
