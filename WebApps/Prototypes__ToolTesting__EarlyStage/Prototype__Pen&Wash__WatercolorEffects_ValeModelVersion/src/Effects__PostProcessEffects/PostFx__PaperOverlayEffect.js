...
// -----------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    const PAPER_OVERLAY_EFFECT_ENABLED           = true;                                         // <-- Enable/disable paper overlay effect
    const PAPER_OVERLAY_TEXTURE_PATH             = 'assets/Test__PaperOverlay__.jpg';            // <-- Path to paper texture image
    const PAPER_OVERLAY_POSTPROCESS_RESOLUTION   = 1.0;                                          // <-- Post-process resolution ratio (1.0 = full resolution)
    const PAPER_OVERLAY_INTENSITY                = 1.0;                                          // <-- Paper overlay intensity (0.0 to 1.0)
    const PAPER_OVERLAY_ASPECT_CORRECTION        = true;                                         // <-- Enable aspect ratio correction
    // ------------------------------------------------------------
    
    // MODULE CONSTANTS | Luminance-Based Masking Configuration
    // ------------------------------------------------------------
    const PAPER_OVERLAY_INTENSITY_MIN            = 0.25;                                         // <-- Paper intensity on bright areas (white background)
    const PAPER_OVERLAY_INTENSITY_MAX            = 1.0;                                          // <-- Paper intensity on dark areas (3D models)
    const PAPER_OVERLAY_LUMINANCE_THRESHOLD      = 0.85;                                         // <-- Brightness threshold (0.0-1.0, higher = more selective)
    const PAPER_OVERLAY_LUMINANCE_CONTRAST       = 2.5;                                          // <-- Mask contrast (higher = sharper transition)
    // ------------------------------------------------------------
    
    // MODULE CONSTANTS | Parallax Effect Configuration
    // ------------------------------------------------------------
    const PAPER_OVERLAY_PARALLAX_ENABLED         = true;                                         // <-- Enable parallax effect for depth perception
    const PAPER_OVERLAY_PARALLAX_SCALE_X         = 0.02;                                         // <-- Horizontal parallax scale (controls side-to-side shift)
    const PAPER_OVERLAY_PARALLAX_SCALE_Y         = 0.01;                                         // <-- Vertical parallax scale (controls up-down shift)
    const PAPER_OVERLAY_TEXTURE_SCALE            = 1.4;                                          // <-- Paper texture tiling scale (higher = more detail/smaller grain)
    // ------------------------------------------------------------
    
    // MODULE CONSTANTS | Shader File Path
    // ------------------------------------------------------------
    const PAPER_OVERLAY_SHADER_PATH              = 'src/Effects__PostProcessEffects/PostFx__PaperOverlayEffect.glsl';  // <-- Path to GLSL shader file
    // ------------------------------------------------------------
...
// -----------------------------------------------------------------------------
    ...
    async function loadPaperOverlayShader() {
        try {
            const response = await fetch(PAPER_OVERLAY_SHADER_PATH);                             // <-- Fetch shader file
...
// -----------------------------------------------------------------------------
    ...
    function createPaperOverlayPostProcess(scene, camera, engine, paperTexture) {
        ...
        const postProcess = new BABYLON.PostProcess(                                              // <-- Create custom post-process
            ...
            PAPER_OVERLAY_POSTPROCESS_RESOLUTION,                                                 // <-- Resolution ratio
        ...
        postProcess.onApply = (effect) => {                                                        // <-- Set up effect uniforms
            ...
            effect.setFloat('paperIntensity', PAPER_OVERLAY_INTENSITY);                            // <-- Set paper intensity
            effect.setFloat('paperScale', PAPER_OVERLAY_TEXTURE_SCALE);                            // <-- Set paper texture scale
            
            effect.setFloat('paperIntensityMin', PAPER_OVERLAY_INTENSITY_MIN);                     // <-- Set minimum intensity (bright areas)
            effect.setFloat('paperIntensityMax', PAPER_OVERLAY_INTENSITY_MAX);                     // <-- Set maximum intensity (dark areas)
            effect.setFloat('luminanceThreshold', PAPER_OVERLAY_LUMINANCE_THRESHOLD);              // <-- Set luminance threshold
            effect.setFloat('luminanceContrast', PAPER_OVERLAY_LUMINANCE_CONTRAST);                // <-- Set luminance contrast
...
// -----------------------------------------------------------------------------
    ...
    function setupParallaxTracking(camera, postProcess) {
        if (!PAPER_OVERLAY_PARALLAX_ENABLED) {                                                    // <-- Check if parallax is enabled
        ...
        const updateParallax = () => {                                                            // <-- Create update function
            ...
            // ------------------------------------
            const totalOffsetX = (currentPosition.x - initialCameraPosition.x) * PAPER_OVERLAY_PARALLAX_SCALE_X;  // <-- Calculate X offset with scale
            const totalOffsetY = (currentPosition.y - initialCameraPosition.y) * PAPER_OVERLAY_PARALLAX_SCALE_Y;  // <-- Calculate Y offset with scale
...
// -----------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    async function setupPaperOverlayEffect(scene, camera, engine, enabled = null) {
        // #Region ------------------------------------------------
        // Effect Enabled Control
        // --------------------------------------------------------
        const effectEnabled = enabled !== null ? enabled : PAPER_OVERLAY_EFFECT_ENABLED;  // <-- Use passed enabled flag or default constant
        // #endregion ---------------------------------------------
        ...
        try {
            ...
            // Load paper texture
            // ------------------------------------
            const paperTexture = await loadPaperTexture(scene, PAPER_OVERLAY_TEXTURE_PATH);       // <-- Load paper texture
            ...
            if (PAPER_OVERLAY_PARALLAX_ENABLED) {
