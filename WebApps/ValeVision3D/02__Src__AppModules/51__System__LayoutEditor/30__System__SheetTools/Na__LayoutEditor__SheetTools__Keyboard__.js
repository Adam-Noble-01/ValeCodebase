// =============================================================================
// VALEVISION3D - LAYOUT EDITOR - SHEET TOOLS - KEYBOARD
// =============================================================================
//
...
//   Escape backs out one step at a time, Space clears, Enter finishes a
//   polyline or content editing, Delete removes, the arrows nudge the
//   selection or lock the drawing axis (AxisKey), the tool keys pick a tool
//   or arm the eyedropper, the snap key toggles snapping, Ctrl+Z and Ctrl+Y
//   step the history (or a polyline's vertices), and Ctrl+C, Ctrl+V, Ctrl+D,
//   Ctrl+G and Ctrl+Shift+G run the clipboard and groups.
...
// - The space bar arms Select, as V does, and both keys are taken from the
//   browser - the space bar would otherwise scroll the page under the sheet.
//   (Ported from TrueVision3D v2.58.0, where it was a toggle; Select is the one
//   resting state, so it no longer toggles off.)
//
//
...
// -----------------------------------------------------------------------------
    ...
    // MODULE IMPORTS | Config, Model, Surface, Tools, Clipboard, Groups, Eyedropper, Axis Lock, Snapping, Selection, History, Confirm Dialog
    // ------------------------------------------------------------
...
// -----------------------------------------------------------------------------
    ...
    function Na__LeTools__AxisKey(axis, shift) {
        ...
        // A VERTEX OR A MEASURED POINT TAKES THE SAME KEYS, for the same reason
        // the keys exist at all. Shift holds a drag to an axis too, but it has to
        // be let go of to reach the number keys and the constraint goes with it,
        // so the typed length lands along whatever direction the raw cursor
    ...
    function Na__LeTools__OnKey(event) {
        ...
        // own, so a Ctrl chord belongs to the sheet.
        ...
        switch (match.action) {
            ...
            // a viewport's content editing finishes - and with one vector, one
            // dimension or one group selected and nothing in flight, Enter opens
            // it, which is the keyboard's way to do what a double click does.
            // ------------------------------------
            case 'Edit__Finish': {
