// =============================================================================
// VALEVISION3D - DRAWING VIEW CORE - MATERIAL PRESET
// =============================================================================
//
...
// - Parity        : new
// - Divergences   : The plan proposed Na__MaterialsSystem__ApplyWhitecardToIndexedMaterials plus the
//                   engine's restore pass; a self-contained stash-and-restore was used instead so exit
//                   never depends on loading-sequence internals (lookup map, environment texture).
// - Back-port     : candidate when TrueVision gains per-drawing style toggles.
//
// -----------------------------------------------------------------------------
//
// DEVELOPMENT LOG:
...
// -----------------------------------------------------------------------------
    ...
    import { Na__RenderEngine__IsMaxEngineActive } from '../05__RenderPipeline/Na__RenderEngine__State.js';
...
// -----------------------------------------------------------------------------
    ...
    // reporting success: the sliding doors keep showing the room behind them
    // with the toggle on.
    //
...
// -----------------------------------------------------------------------------
    ...
    function Na__DrawView__MaterialPreset__Enter(styles) {
        ...
        if (Na__DrawMat__Styles.whitecard && Na__RenderEngine__IsMaxEngineActive()) {
        ...
        if (swapped > 0) console.log('[ValeVision3D] Drawing material preset swapped ' + swapped + ' material slot(s).');
