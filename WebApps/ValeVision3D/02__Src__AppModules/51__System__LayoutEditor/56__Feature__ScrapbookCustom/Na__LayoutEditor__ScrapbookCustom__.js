// =============================================================================
// VALEVISION3D - LAYOUT EDITOR - CUSTOM SCRAPBOOK
// =============================================================================
//
        ...
        if (!Na__LeScrapCustom__LoadPromise) {
            Na__LeScrapCustom__LoadPromise = (async () => {
                try {
                    ...
                    console.warn('[ValeVision3D LayoutEditor] Custom Scrapbook config unavailable - its defaults are used.', error);
...
// -----------------------------------------------------------------------------
    ...
    async function Na__LeScrapCustom__LoadCategory(folder) {
        ...
        reads.forEach((read, i) => {
            if (!read.ok) console.warn('[ValeVision3D LayoutEditor] A custom scrapbook item could not be read: ' + wanted[i].Item__File + ' (' + read.error + ')');
...
// -----------------------------------------------------------------------------
    ...
    function Na__LeScrapCustom__Capture(sheet, selectionItems) {
        ...
        doc[Na__LeScrapCustom__DOC + 'Meta'] = {
            Meta__Description : 'A Custom Scrapbook item for the ValeVision 3D Layout Editor: a selection saved from a sheet. Entries are sheet records in paper millimetres, y down, from an origin at the top left of the item; Roots is what a drop selects. Written by the app through the local development server; see Na__LayoutEditor__ScrapbookCustom__.js.',
