// =============================================================================
// VALEVISION3D - LAYOUT EDITOR - SHEET TOOLS - POINTER PRESS
// =============================================================================
//
...
// PURPOSE    : What a press on the stage does (place, pick a style, start a box, change the selection, start a drag), and the double click
// CREATED    : 15-Sep-2026
//
// DESCRIPTION:
...
//   finishes content editing. Where nothing can move it starts a selection
//   box (StartsBox); a Shift-click on an edge of the one selected vector
//   inserts a vertex and drags it; otherwise the selection modifiers apply
//   (PressSelection) and the drag for what was pressed begins (DragFor), or a
//   group drag for several selected items. A change that waits for a click
//   runs on the release, only if the pointer never moved.
// - OnDoubleClick: finishes a polyline being drawn, edits the text, the
//   dimension value or the leader text under the pointer, or enters or leaves
//   a viewport's content.
//
// INTEGRATION:
...
//   unchanged - they edit an object rather than relocate it.
...
// -----------------------------------------------------------------------------
    ...
    // MODULE IMPORTS | Config, Model, Surface, Handles, Markup, Grips, Tools, Selection, Menu
    // ------------------------------------------------------------
...
// -----------------------------------------------------------------------------
    ...
    // FUNCTION | Step Inside a Container (a group, a vector or a dimension)
    // ------------------------------------------------------------
    // Entering a vector or a dimension selects it, so its highlight and its
    // points are on screen straight away. Entering a group selects nothing: the
    // point of being inside is to pick its members, and arriving with the group
    // still selected would mean the first thing done inside it moved the lot.
    // Repositioning a viewport's content is a container of its own and cannot be
    ...
    function Na__LeTools__OnDown(event) {
        ...
        if (!drag) { if (click) click(); return; }   // <-- The press is a plain pick: with Select up a drag moves nothing
        drag.startMm   = point;
        drag.moved     = false;
