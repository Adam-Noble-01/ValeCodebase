// =============================================================================
// VALEVISION3D - NORTH DIRECTION - CONFIG STATE
// =============================================================================
//
...
//
// PORT NOTE:
// - Ported from   : TrueVision3D 46__System__NorthDirection/Na__North__ConfigState__.js,
//                   its 1.0.0 (TrueVision v2.85.0)
// - Ported on     : 20-Sep-2026 for ValeVision3D v2.67.0
// - Parity        : verbatim (the Gizmo setup still reads ShownByDefault, which this app's config does not carry, and answers false)
...
// -----------------------------------------------------------------------------
    ...
    async function Na__NorthCfg__Fetch() {
        try {
            ...
            if (!response.ok) {
                console.warn('[ValeVision3D] North direction config fetch failed (' + response.status + ') - using built-in defaults.');
                return false;
            }
            Na__NorthCfg__Config = await response.json();
            return Na__NorthCfg__IsEnabled();
        } catch (error) {
            console.warn('[ValeVision3D] North direction config unreadable - using built-in defaults.', error);
            return false;
        }
    }
    // ------------------------------------------------------------
