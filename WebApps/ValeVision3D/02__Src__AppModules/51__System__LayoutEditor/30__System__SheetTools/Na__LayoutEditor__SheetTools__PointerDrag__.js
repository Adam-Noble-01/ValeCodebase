// =============================================================================
// VALEVISION3D - LAYOUT EDITOR - SHEET TOOLS - POINTER DRAG
// =============================================================================
//
...
//   Select tool's hover cursor and insert diamond.
...
//   viewport (moved, cropped or its drawing panned), text (moved or turned),
//   a shape (a vertex, or the whole shape snapped by a vertex or its grab
//   point), a leader (the tip, the head or the whole) or a dimension (an end,
//   the line's offset, the value or the whole).
...
//   closes the drag and announces the change once, or runs the press's click
...
//   the rest move by exactly however far it went, so a boxed run of corners
//   keeps its shape (drag.indices, from the edit scope).
...
// -----------------------------------------------------------------------------
    ...
    // MODULE IMPORTS | Config, Model, Surface, Handles, Grips, Tools, Viewports, Snapping, Selection
    // ------------------------------------------------------------
...
// -----------------------------------------------------------------------------
    ...
    function Na__LeTools__OnMove(event) {
        ...
        if (!drag || event.pointerId !== drag.pointerId) {
            ...
            Na__LeTools__Stage.style.cursor = inserting ? 'crosshair' : Na__LeTools__HoverCursor(sheet, found, point);
            return;
        }
    ...
    function Na__LeTools__ApplyDrag(sheet, drag, dMm, shift, exact) {
        ...
        if (drag.kind === 'shape') {
            ...
            if (drag.mode === 'vertex') {
                ...
                // pointer is the one that snaps; the rest move by exactly
                // however far it went, so a boxed run of corners keeps its
                // shape. On its own it is the single-vertex drag that has
                // always been here.
    ...
    // ------------------------------------------------------------
    // The way out of a vector, a dimension and a group: click off it. The press
    // is spent on leaving - it does not also select whatever it landed on, which
    ...
    function Na__LeTools__FinishDrag(pointerId, released) {
        ...
        const drag = Na__LeTools__Drag;
        if (!drag) return;
        // ONLY A VERTEX OR DIMENSION DRAG'S OWN BAND AND LOCK GO HERE. Both are
        // shared with the tools that are placing points, and a drag finishing is
        // no reason to take a half-drawn polyline's band away.
