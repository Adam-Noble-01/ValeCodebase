// =============================================================================
// VALEVISION3D - LAYOUT EDITOR - DIMENSION GEOMETRY
// =============================================================================
//
...
//
// PORT NOTE:
// - Ported from   : ValeVision3D 45__System__PlanDimensions/Na__PlanDimensions__Overlay__.js (skeleton, terminators, text placement)
// - Ported on     : 09-Sep-2026 for ValeVision3D v2.21.0 (port Phase 5)
// - Parity        : adapted
// - Divergences   : primitives instead of live SVG nodes; no hit line; the dot is an octagon (the primitive set has no circle).
// - Back-port     : none.
// - TrueVision    : 1.1.0 (ortho orientations) was authored in TrueVision3D v2.31.0 and
//                   ported back 13-Sep-2026; 1.2.0 (text leader, justified side,
//                   outward hook) was authored in TrueVision3D and ported back
//                   14-Sep-2026. Fixed-length extension lines stay there.
//
// -----------------------------------------------------------------------------
//
// DEVELOPMENT LOG:
...
//   from the justified side of the row back to MID, bulging away from the
//   dimension line so the hook bends outwards. Push draws that arc under the
//   value. HitText and DistanceToPolyline hit-test it. Without an offset a
//   dimension is unchanged to the bit.
// - Ported from TrueVision3D (DimensionGeometry 1.3.0 to 1.5.1).
//
...
// -----------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    function Na__LeDimGeo__Skeleton(start, end, offsetMm, gapMm, overshootMm, orientation) {
        ...
        return {
            ...
            DS  : DS,
            DE  : DE,
            X1  : at(start.x, start.y, gapMm * sign),
            T1  : at(start.x, start.y, offset + (overshootMm * sign)),
            X2  : at(end.x,   end.y,   gapMm * signE),
...
// -----------------------------------------------------------------------------
    ...
    //         textDXMm, textDYMm, textLeaderMinMm, textLeaderGapMm, textFillColour }
    ...
    function Na__LeDimGeo__Push(list, spec) {
        const sk = Na__LeDimGeo__Skeleton(spec.start, spec.end, spec.offsetMm, spec.gapMm, spec.overshootMm, spec.orientation);
        if (!sk) return null;
