// =============================================================================
// VALEVISION3D - LAYOUT EDITOR - SPECIFICATION DATA
// =============================================================================
//
...
//   project.json as ValeVision__DrawingNotes__.json, and
...
//                                        so an agent can edit it on disk.
...
//   ValeVision__DrawingNotes__.json, and asks first when the cloud copy changed
...
// - ValeVision    : ported 14-Sep-2026. Transport reads/writes ValeVision__DrawingNotes__.json
//                   beside project.json via the Whitecardopedia Flask + R2 worker.
//
// -----------------------------------------------------------------------------
//
// DEVELOPMENT LOG:
// 14-Sep-2026 - Version 1.1.0
// - Canonical file ValeVision__DrawingNotes__.json, kept locally beside the
//   project.json and written to the same R2 key on Sync. A newer local
//   UpdatedIso is adopted so an on-disk edit reaches the editor.
//
...
// -----------------------------------------------------------------------------
    ...
    // MODULE IMPORTS | Config, Project Code, Environment, Drawing Notes and the Confirm Dialog
    // ------------------------------------------------------------
...
// -----------------------------------------------------------------------------
    ...
    const Na__LeSpec__DESCRIPTION   = 'Project specification notes for the ValeVision drawings. Groups hold notes in order; a note’s code is its group’s prefix and its place in the group (GN01, EE02), so the order is the truth and Note__Code is written for readers and recomputed on every load. Specification bubbles on the Layout Editor sheets link to notes by Note__Id. General groups are listed on every sheet’s notes margin, after the notes that sheet’s bubbles link to. LastIdNumber only ever goes up, so no id is used twice.';
    // ------------------------------------------------------------
...
// -----------------------------------------------------------------------------
    ...
    function Na__LeSpec__RestoreDraft() {
        ...
        console.log('[ValeVision3D] Layout Editor: unsynced specification draft restored from this browser.');
        return true;
    }
    // ------------------------------------------------------------
