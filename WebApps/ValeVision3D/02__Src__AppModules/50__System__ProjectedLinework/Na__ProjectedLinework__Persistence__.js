...
//
// PORT NOTE:
// - Ported from   : Lantern Designer 02__Src__AppModules/27__System__ProjectedEdges2d/VghLantern__ProjectedEdges__LineworkStore__.mjs
// - Source version: Lantern Designer rebuild of 07-Aug-2026
// - Ported on     : 09-Sep-2026 for ValeVision3D v2.20.0 (port Phase 4)
// - Parity        : adapted
// - Divergences   :
//   - One block per drawing as an R2 asset (Na__AppUtils__R2AssetUpload) with a record reference, not a lantern block.
//   - Four line classes per block; IndexedDB copy on the device; bake-before-save.
...
//   not upload an untagged result. Schema 1 assets and browser copies are
//   re-rendered once: a drawing restored from one would leave the Layout
//   Editor's edge styles doing nothing. Ported from TrueVision3D (Edge Styles).
//
...
// -----------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    const Na__PlStore__DB_NAME  = 'ValeVision3D__ProjectedLinework';
...
// -----------------------------------------------------------------------------
    ...
    function Na__PlStore__Serialise(definition, classes, meta) {
        ...
        for (let i = 0; i < Na__PlStore__CLASSES.length; i++) {
            ...
            if (count > setup.maxSegmentsPerView) {
                console.warn('[ValeVision3D ProjectedLinework] ' + definition.ViewKey + ' ' + name + ' has ' + count +
        ...
        return {
            Meta : {
                Description        : 'Projected linework for one ValeVision drawing, rendered from the model and stored so the web build never computes it. Coordinates are drawing millimetres, x right and y down, four numbers per segment: x0, y0, x1, y1. A plan reads world X and world Z; an elevation reads the run along the facade and minus the height. Ignore the block if Fingerprint does not match the drawing and model you hold.',
    ...
    function Na__PlStore__Deserialise(block, fingerprint) {
        ...
        if (block.Meta.SchemaVersion !== Na__PlStore__SCHEMA_VERSION) {
            console.info('[ValeVision3D ProjectedLinework] Stored linework is schema v' + block.Meta.SchemaVersion + '; ignoring it.');
            return null;
        }
        if (block.Meta.ModelBuildToken !== Na__PlCfg__GetModelSetup().buildToken) {
            console.info('[ValeVision3D ProjectedLinework] Stored linework was sampled by build "' + block.Meta.ModelBuildToken + '"; ignoring it.');
            return null;
        }
        if (!fingerprint || block.Meta.Fingerprint !== fingerprint) {
            console.info('[ValeVision3D ProjectedLinework] Stored linework belongs to an earlier state of this drawing; ignoring it.');
            return null;
        }
        ...
        if (!allTagged || !Na__PlOwners__Attach(classes, restored, block.Meta.OwnerKeys)) {
            console.info('[ValeVision3D ProjectedLinework] Stored linework for ' + block.Meta.ViewKey + ' carries no category tags; ignoring it so it re-renders with them.');
            return null;
        }
        return classes;
    }
...
// -----------------------------------------------------------------------------
    ...
    async function Na__PlStore__BakeOne(definition, modelFingerprint, showToast, force) {
        ...
        if (!Na__PlOwners__Has(classes)) {
            console.warn('[ValeVision3D ProjectedLinework] ' + definition.ViewKey + ' rendered without category tags; not baked.');
            return 'refused';
        }
    ...
    async function Na__PlStore__BakeAll(options) {
        ...
        for (let i = 0; i < definitions.length; i++) {
            ...
            try {
                ...
                console.error('[ValeVision3D ProjectedLinework] Bake failed for ' + definition.ViewKey + ':', bakeError);
                counts.failed++;
            }
        }
        return counts;
    ...
    async function Na__PlStore__BakeBeforeSave(showToast, includeDrawingIds) {
        ...
        try {
            ...
            console.error('[ValeVision3D ProjectedLinework] Bake before save failed:', bakeError);
            return null;
        }
    }
    // ------------------------------------------------------------
