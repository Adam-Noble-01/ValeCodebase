// =============================================================================
// VALEVISION3D - PROJECTED LINEWORK - PROJECTOR
// =============================================================================
//
...
//
// PORT NOTE:
// - Ported from   : Lantern Designer 02__Src__AppModules/27__System__ProjectedEdges2d/VghLantern__ProjectedEdges__Projector__.mjs
// - Source version: Lantern Designer rebuild of 07-Aug-2026
// - Ported on     : 09-Sep-2026 for ValeVision3D v2.20.0 (port Phase 4)
// - Parity        : adapted
// - Divergences   :
//   - Basis from the view definition instead of a fixed table; ScaleDivisor is the ValeVision unit (0.001).
//   - Preparation split into a per-model-state collection and a per-drawing sample (the cut).
//   - Options carry IncludeHiddenEdges from the drawing's styles (D21).
...
//   the Layout Editor's per-category edge styles would have done nothing.
// - The collection carries an OwnerTable built from its instance list, shared by
//   the per-view stage pass and the cached intersection pass. Ported from TrueVision3D (Edge Styles).
//
...
// -----------------------------------------------------------------------------
    ...
    function Na__PlProjector__NoteTaggedCpu(requested) {
        ...
        console.info('[ValeVision3D ProjectedLinework] Kept linework renders on the CPU backend (configured: ' + requested + '). '
    ...
    function Na__PlProjector__ResolveBackend(requested, definition, diagnostic) {
        ...
        if (requested === Na__PlProjector__BACKEND_WEBGPU) {
            if (hasCut) {
                console.info('[ValeVision3D ProjectedLinework] WebGPU was asked for, but this view has a drawing cut '
            ...
            console.info('[ValeVision3D ProjectedLinework] WebGPU was asked for but is not usable here ('
    ...
    async function Na__PlProjector__Project(collected, sampled, definition, options, run) {
        if (options.Backend === Na__PlProjector__BACKEND_WEBGPU) {
            try {
                ...
                console.warn('[ValeVision3D ProjectedLinework] WebGPU projection failed, falling back to the CPU backend:', gpuError);
            }
        }
