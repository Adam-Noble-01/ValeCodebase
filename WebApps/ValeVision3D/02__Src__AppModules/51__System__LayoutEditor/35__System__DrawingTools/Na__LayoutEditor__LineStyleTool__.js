// =============================================================================
// VALEVISION3D - LAYOUT EDITOR - LINE STYLE TOOL
// =============================================================================
//
...
//
// PORT NOTE:
// - Ported from   : TrueVision3D 51__System__LayoutEditor/Na__LayoutEditor__LineStyleTool__.js
// - Ported on     : 14-Sep-2026 for ValeVision3D v2.40.0 (a TrueVision-first feature)
// - Parity        : verbatim
// - Divergences   : Header and console prefix only. The config JSON is verbatim and
//                   the record shape is shared, so either app reads the other's line styles.
...
// - Ported from TrueVision3D: the record, the four kinds, the scale, the
//   millimetre fields, the Vectors panel rows and the paper-millimetre
//   pattern the chrome paints. Config JSON identical.
//
// =============================================================================
        ...
        if (!Na__LeDash__LoadPromise) {
            Na__LeDash__LoadPromise = (async () => {
                try {
                    ...
                    if (!response.ok) {
                        console.warn('[ValeVision3D LayoutEditor] Line style config fetch failed (' + response.status + ') - the built-in defaults will be used.');
                        return null;
                    }
                    Na__LeDash__Config = await response.json();
                } catch (error) {
                    console.warn('[ValeVision3D LayoutEditor] Line style config unavailable - the built-in defaults will be used.', error);
                }
