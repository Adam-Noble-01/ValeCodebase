// =============================================================================
// VALEVISION3D - PROJECTED LINEWORK - CONFIG ACCESS
// =============================================================================
//
...
// -----------------------------------------------------------------------------
    ...
    async function Na__PlCfg__Fetch() {
        try {
            ...
            if (!response.ok) {
                console.warn('[ValeVision3D ProjectedLinework] Config fetch failed (' + response.status + ') - the overlay stays off.');
                return false;
            }
            Na__PlCfg__Config = await response.json();
            return true;
        } catch (error) {
            console.warn('[ValeVision3D ProjectedLinework] Config unreadable - the overlay stays off.', error);
            return false;
        }
    }
    // ------------------------------------------------------------
