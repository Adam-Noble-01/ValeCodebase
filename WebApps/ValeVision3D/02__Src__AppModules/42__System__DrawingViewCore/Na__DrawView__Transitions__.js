...
//                   (regions "Suspending the 3D Systems" and the flight calls, shared with the elevation controller)
// - Source version: 1.1.0 (07-Sep-2026)
// - Ported on     : 09-Sep-2026 for ValeVision3D v2.18.0 (port Phase 2)
// - Parity        : adapted (split)
// - Divergences   :
//   - Walk and fly are returned to orbit through the navigation toolbar, which TrueVision has no need of.
//   - Distance culling lives under the MaxEngine folder in ValeVision.
...
// -----------------------------------------------------------------------------
    ...
    import { Na__NavToolbar__SetOrbitMode } from '../10__NavigationAndCameras/Na__UiFeature__NavigationToolbar__Controls.js';
    // ------------------------------------------------------------
...
// -----------------------------------------------------------------------------
    ...
    function Na__DrawView__Transitions__SuspendThreeD() {
        if (Na__WalkMode__IsActive() || Na__FlyMode__IsActive()) Na__NavToolbar__SetOrbitMode(); // <-- Safe mode conversion first
