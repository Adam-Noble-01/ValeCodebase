// =============================================================================
// VALEVISION3D - LAYOUT EDITOR - SHEET MODEL - COMMON TITLE BLOCK FIELDS
// =============================================================================
//
...
//
// DESCRIPTION:
// - A project has one client and one site. Every sheet in the pack prints the
//   same two lines, and the Layout Editor used to store them per sheet - which
//   meant typing them again on every new drawing, and meant a pack could end up
//   disagreeing with itself over a trailing space or a spelling.
...
//   Off, the sheet keeps its own, for the one drawing that needs a different
//   address.
//
...
//   NOT from the project record. An issued drawing must not quietly change its
//   printed address because a project file spells a county differently - the
//   sheets are the truth about what was issued, and the record is then offered
//   beside them to take or leave.
// - A project with nothing typed anywhere - a new one - seeds straight from the
//   record, which is the whole point of the feature.
//
// INTEGRATION:
// - Na__LayoutEditor__SheetRecords__ reads Get and Uses for its field defaults.
// - Na__LayoutEditor__Panel__Sheet__ draws the switch and the two rows.
// - Na__LayoutEditor__ProjectRecord__ supplies the project record.
// - Na__LayoutEditor__History__ carries Get on every step, because these two
//   values are the only editable thing in the editor that is NOT on a sheet.
//
// -----------------------------------------------------------------------------
//
// PORT NOTE:
// - Ported from   : TrueVision3D 51__System__LayoutEditor/07__Core__SheetData/Na__LayoutEditor__SheetModel__Common__.js 1.0.0
// - Ported on     : 20-Sep-2026 for ValeVision3D (Common title block fields)
// - Parity        : verbatim (the drawings-block path differs, as it does for every
//                   module in this folder: 42__ here, 40__ in TrueVision)
// - Divergences   : none in this file. Where the record comes from is
//                   Na__LayoutEditor__ProjectRecord__ and is adapted there.
// - Back-port     : n/a
//
// -----------------------------------------------------------------------------
//
...
// -----------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    import { Na__DrawData__GetCommonFields, Na__DrawData__SetCommonField } from '../../42__System__DrawingViewCore/Na__DrawView__ProjectData__.js';
