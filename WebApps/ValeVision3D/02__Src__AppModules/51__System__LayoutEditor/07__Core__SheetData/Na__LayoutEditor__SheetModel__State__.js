// =============================================================================
// VALEVISION3D - LAYOUT EDITOR - SHEET MODEL - STATE
// =============================================================================
//
...
//   types, the scenes key and the style keys) and the session state: the
//   active sheet id, the selection items and the dirty flag.
...
// - RegisterBeforeAnnounce, ported from TrueVision3D's State unit 1.1.0: a
//   feature can bring what it derives from a sheet up to date, silently, BEFORE
//   a change is announced, so the one announcement - and the history's one
//   snapshot of it - already holds it. The Parametric Scrapbook's viewport link
//   uses it: a scale bar follows its viewport's scale, and a drawing title its
//   viewport's name, inside the same undo step. It cannot be done with a
//   listener: the change event is dispatched ON window, and for an event whose
//   target is window the listeners fire in the order they were added, whatever
//   their phase, so the history - added first - has taken its snapshot already.
//
...
// -----------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    import { Na__DrawData__SHEETS_KEY, Na__DrawData__GetBlock } from '../../42__System__DrawingViewCore/Na__DrawView__ProjectData__.js';
    // ------------------------------------------------------------

    // MODULE IMPORTS | Record Kinds, Layer Types and Style Keys
    // ------------------------------------------------------------
    import {
        Na__LeRec__KIND_2D,
        Na__LeRec__KIND_3D,
