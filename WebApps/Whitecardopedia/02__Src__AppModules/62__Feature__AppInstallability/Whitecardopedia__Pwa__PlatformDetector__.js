// =============================================================================
// WHITECARDOPEDIA - PWA PLATFORM DETECTOR
// =============================================================================
//
// FILE       : Whitecardopedia__Pwa__PlatformDetector__.js
// NAMESPACE  : Whitecardopedia
// MODULE     : Whitecardopedia__Pwa__PlatformDetector
...
//   so we fall back to maxTouchPoints to disambiguate).
// - Detects in-app standalone mode using both the modern display-mode media
//   query and the legacy iOS `navigator.standalone` flag.
...
// -----------------------------------------------------------------------------
    ...
    const PLATFORM_ID_INSTALLED_STANDALONE      = 'installed-standalone';                                                           // <-- Already running in standalone PWA mode
    const PLATFORM_ID_UNKNOWN                   = 'unknown';                                                                        // <-- Fallback for unrecognised UAs
    // ------------------------------------------------------------


    // MODULE CONSTANTS | UA Substring Patterns
    // ------------------------------------------------------------
...
// -----------------------------------------------------------------------------
    ...
    // ---------------------------------------------------------------
    function Whitecardopedia__Pwa__PlatformDetector__GetUserAgent() {
    ...
    // ---------------------------------------------------------------
    function Whitecardopedia__Pwa__PlatformDetector__GetPlatformString() {
    ...
    // ---------------------------------------------------------------
    function Whitecardopedia__Pwa__PlatformDetector__GetMaxTouchPoints() {
    ...
    // ---------------------------------------------------------------
    function Whitecardopedia__Pwa__PlatformDetector__IsStandaloneDisplay() {
        ...
        return Boolean(isDisplayStandalone || isDisplayMinimalUi || isDisplayFullscreen || isDisplayWindowOverlay || isIosLegacyStandalone); // <-- Any standalone-like signal
    }
    // ---------------------------------------------------------------
    ...
    // ---------------------------------------------------------------
    function Whitecardopedia__Pwa__PlatformDetector__IsIpadOsAsMac() {
        const platformString    = Whitecardopedia__Pwa__PlatformDetector__GetPlatformString();                                      // <-- Read platform string
        const maxTouchPoints    = Whitecardopedia__Pwa__PlatformDetector__GetMaxTouchPoints();                                      // <-- Read touch point count
        const userAgent         = Whitecardopedia__Pwa__PlatformDetector__GetUserAgent();                                           // <-- Read user agent
    ...
    // ---------------------------------------------------------------
    function Whitecardopedia__Pwa__PlatformDetector__IsAnyIosDevice() {
        const userAgent         = Whitecardopedia__Pwa__PlatformDetector__GetUserAgent();                                           // <-- Read user agent
        ...
        if (Whitecardopedia__Pwa__PlatformDetector__IsIpadOsAsMac()) return true;                                                   // <-- Modern iPadOS quirk
    ...
    // ---------------------------------------------------------------
    function Whitecardopedia__Pwa__PlatformDetector__IsIpadDevice() {
        const userAgent         = Whitecardopedia__Pwa__PlatformDetector__GetUserAgent();                                           // <-- Read user agent
        if (UA_PATTERN_IPAD.test(userAgent)) return true;                                                                           // <-- Legacy iPad UA match
        if (Whitecardopedia__Pwa__PlatformDetector__IsIpadOsAsMac()) return true;                                                   // <-- iPadOS Mac UA quirk
        return false;                                                                                                               // <-- Otherwise not iPad
    }
    // ---------------------------------------------------------------


    // HELPER FUNCTION | Detect WebKit Engine on iOS Non-Safari Browser
    // ---------------------------------------------------------------
    function Whitecardopedia__Pwa__PlatformDetector__IsIosNonSafariBrowser(userAgent) {
    ...
    // ---------------------------------------------------------------
    function Whitecardopedia__Pwa__PlatformDetector__IsChromiumFamily(userAgent) {
    ...
    // ---------------------------------------------------------------
    function Whitecardopedia__Pwa__PlatformDetector__IsMacSafari(userAgent) {
        const looksLikeMac      = UA_PATTERN_MAC.test(userAgent) && !Whitecardopedia__Pwa__PlatformDetector__IsIpadOsAsMac();       // <-- Real macOS, not iPadOS quirk
        if (!looksLikeMac) return false;                                                                                            // <-- Bail if not macOS
        if (Whitecardopedia__Pwa__PlatformDetector__IsChromiumFamily(userAgent)) return false;                                      // <-- Chromium variants are not Safari
    ...
    // HELPER FUNCTION | Detect Browser Engine for Identifier Output
    // ---------------------------------------------------------------
    function Whitecardopedia__Pwa__PlatformDetector__GetBrowserEngineDescriptor(userAgent) {
        if (UA_PATTERN_EDGE.test(userAgent)) return 'edge';                                                                         // <-- Edge variants (desktop, Android, iOS)
...
// -----------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    function Whitecardopedia__Pwa__PlatformDetector__GetPlatformDescriptor() {
        const userAgent             = Whitecardopedia__Pwa__PlatformDetector__GetUserAgent();                                       // <-- Snapshot UA once
        const isStandaloneNow       = Whitecardopedia__Pwa__PlatformDetector__IsStandaloneDisplay();                                // <-- Standalone runtime flag
        const isAnyIosDevice        = Whitecardopedia__Pwa__PlatformDetector__IsAnyIosDevice();                                     // <-- iOS family flag
        const isIpadDevice          = Whitecardopedia__Pwa__PlatformDetector__IsIpadDevice();                                       // <-- iPad-specific flag
        const browserEngineLabel    = Whitecardopedia__Pwa__PlatformDetector__GetBrowserEngineDescriptor(userAgent);                // <-- Browser label
        ...
        if (isStandaloneNow) {
            ...
            if (Whitecardopedia__Pwa__PlatformDetector__IsIosNonSafariBrowser(userAgent)) {
        ...
        else if (Whitecardopedia__Pwa__PlatformDetector__IsMacSafari(userAgent)) {
            platformId              = PLATFORM_ID_MAC_SAFARI;                                                                      // <-- macOS Safari
        }
        else if (Whitecardopedia__Pwa__PlatformDetector__IsChromiumFamily(userAgent)) {
    ...
    // ------------------------------------------------------------
    function Whitecardopedia__Pwa__PlatformDetector__SubscribeStandaloneChanges(callback) {
        ...
        const handler           = (event) => callback(Boolean(event.matches));                                                      // <-- Forward boolean status

        if (mediaQueryList.addEventListener) {
            mediaQueryList.addEventListener('change', handler);                                                                     // <-- Modern listener API
            return () => mediaQueryList.removeEventListener('change', handler);                                                     // <-- Disposer
        }

        if (mediaQueryList.addListener) {
            mediaQueryList.addListener(handler);                                                                                    // <-- Legacy listener API
            return () => mediaQueryList.removeListener(handler);                                                                    // <-- Legacy disposer
        }
...
// -----------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    function Whitecardopedia__Pwa__PlatformDetector__InitializeGlobalNamespace() {
        if (typeof window === 'undefined') return;                                                                                  // <-- Guard non-window contexts

        window.Whitecardopedia__Pwa__PlatformDetector = {                                                                           // <-- Public API surface
            getPlatformDescriptor      : Whitecardopedia__Pwa__PlatformDetector__GetPlatformDescriptor,
            subscribeStandaloneChanges : Whitecardopedia__Pwa__PlatformDetector__SubscribeStandaloneChanges,
            isStandaloneDisplay        : Whitecardopedia__Pwa__PlatformDetector__IsStandaloneDisplay,
            PlatformIds : {
    ...
    Whitecardopedia__Pwa__PlatformDetector__InitializeGlobalNamespace();                                                            // <-- Mount on window immediately
