#!/usr/bin/env python3
# =============================================================================
# VALEVISION3D - TEST - CUSTOM SCRAPBOOK API
# =============================================================================
#
...
#
# DESCRIPTION:
# - Drives WebApps/Whitecardopedia/Server__ValeVisionScrapbook__Api__.py through
...
#   Writes no byte-code: the repository tracks that copy's __pycache__ files,
#   and a run must leave the working tree as it found it.
#
# -----
#
# PORT NOTE:
# - Ported from   : TrueVision3D 80__Testing__PrototypeEnvironment/Na__Test__ScrapbookApi__.test.py, its 1.0.0
# - Ported on     : 20-Sep-2026 for ValeVision3D v2.69.0
# - Parity        : adapted - the module under test, its route and where it is found; every check is the source's
#
# -----
#
# DEVELOPMENT LOG:
...
sys.dont_write_bytecode = True                                      # <-- THE REPOSITORY TRACKS the bundled dependencies' __pycache__ files: importing them from here must not rewrite 108 of them
...
import Server__ValeVisionScrapbook__Api__ as api                   # noqa: E402
from flask import Flask                                             # noqa: E402

ROUTE     = '/api/valevision/scrapbook'
...
app = Flask(__name__)
app.register_blueprint(api.valevision_scrapbook_api)
...
print('\nValeVision3D - custom scrapbook api\n')
...
for name, payload in (
    ...
    ('a traversal category is refused',            {'category': '../../Whitecardopedia', 'name': 'x1', 'item': item_document()}),
...
for name, payload in (
    ('a traversal delete is refused',                {'file': '../../Whitecardopedia/server.py'}),
