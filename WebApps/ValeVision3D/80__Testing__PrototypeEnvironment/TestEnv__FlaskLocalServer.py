# =============================================================================
# VALEVISION3D - TEST ENVIRONMENT FLASK SERVER
# =============================================================================
#
...
#
# DESCRIPTION:
# - Lightweight Flask server dedicated to the ValeVision3D testing environment.
# - Serves the test sandbox HTML/JS/CSS from the local folder.
# - Serves engine scripts from the parent ValeVision3D directory (no duplication).
...
# - GET  /<path>                                 : Fallback to ValeVision3D root
#
# =============================================================================
...
VALEVISION_ROOT        = os.path.dirname(SCRIPT_DIR)                         # <-- Parent ValeVision3D folder
WEBAPPS_ROOT           = os.path.dirname(VALEVISION_ROOT)                    # <-- WebApps folder (contains shared assets)
...
SERVER_HOST            = '127.0.0.1'                                         # <-- Localhost binding
    ...
    # ------------------------------------------------------------
app = Flask(__name__, static_folder=VALEVISION_ROOT)                         # <-- Static root is ValeVision3D parent
...
def serve_shared_assets(filename):
    ...
    # ROUTE HANDLER | Catch-All for ValeVision3D Engine Files
    # ------------------------------------------------------------
@app.route('/<path:filename>', methods=['GET'])
def serve_engine_files(filename):
    """Fallback: serve engine scripts and styles from ValeVision3D root"""
    try:
        file_path = os.path.join(VALEVISION_ROOT, filename)                  # <-- Build full path

        if os.path.isfile(file_path):
            return send_from_directory(VALEVISION_ROOT, filename)            # <-- Serve from ValeVision3D root
        else:
            return jsonify({
