...
// -----------------------------------------------------------------------------
    ...
    function Na__AmbientOcclusion__GenerateKernel(sampleCount) {
        ...
        for (let i = 0; i < sampleCount; i++) {
            ...
            // contributes more - but that weighting is applied per frame against
            // the number of samples the frame is actually walking
            // (uAoActiveSamples), not baked into the vector lengths here. Baked
            // in, a frame on a reduced budget would inherit a PREFIX of the
            // weighting - every sample bunched against the surface, a hard contact
            // line with none of the falloff - and the moving image would not match
            // the still it settles into.
            kernel.push(sample);
        }
        return kernel;
    }
...
// -----------------------------------------------------------------------------
    ...
    function Na__RenderEffect__AmbientOcclusion__Create(camera, aoConfig, depthTexture) {
        ...
        // SUB FUNCTION | Quality Budget, Borrowed for a Draw and Always Put Back
        // ------------------------------------------------------------
