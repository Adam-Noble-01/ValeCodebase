// =============================================================================
// VALEVISION3D - PRESENTATION MODE - SCENE GROUPS DATA LAYER
// =============================================================================
//
...
//   sets (Exterior 3D Views, Interior 3D Views, Floor Plans, Elevations, Cross
//   Sections...) so the carousel shows one coherent set at a time rather than
//   every scene the project has ever had.
// - Group definitions live INSIDE the project's existing
//   PresentationMode__SavedCameraScenes block, under the __Groups key, because
//   they are scene-related data (plan decision D08). The Whitecardopedia sync
//   pipeline only patches its own keys inside that block (camera data, image
//   list, IMG-slot thumbnail names), so groups survive a SketchUp re-sync, and
//   every Dev menu save already writes the whole block through the R2-first
//   utility, so nothing new is needed to persist them.
...
//   index.html through the selector bar; it loads this module's own AppConfig
//   JSON.
...
//   - The configured default group set gains a sixth group, Cross Sections (plan decision D09).
...
// -----------------------------------------------------------------------------
    ...
    async function Na__PmGroups__FetchConfig() {
        try {
            ...
            if (!response.ok) {
                console.warn(`[ValeVision3D] Scene groups config fetch failed (${response.status}) - grouping disabled.`);
                return false;
            }
            Na__PmGroups__Config = await response.json();
            return Na__PresentationMode__SceneGroups__IsEnabled();
        } catch (error) {
            console.warn('[ValeVision3D] Scene groups config unreadable - grouping disabled.', error);
