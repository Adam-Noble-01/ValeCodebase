...
// - THE COMPASS IS ONLY IN THE SCENE WHILE THE PANEL IS OPEN, and is disposed
//   the moment it closes - and the panel closes itself when a Layout Editor
//   sheet opens, because a compass in the scene would be drawn onto whatever
//   that sheet renders. It also steps out for as long as any sheet picture is
//   queued or rendering (a scene saved from the 3D tab restamps its viewports)
//   and comes back after. TrueVision's Show Compass toggle is not here: it
//   rests on an interactive-overlay registry this app does not have yet.
//
// INTEGRATION:
...
//
// PORT NOTE:
// - Ported from   : TrueVision3D 46__System__NorthDirection/Na__North__DevMenu__Editor__.js,
//                   its 1.1.0 (TrueVision v2.85.0)
// - Ported on     : 20-Sep-2026 for ValeVision3D v2.67.0
// - Parity        : adapted
// - Divergences   : No Show Compass button: keeping the compass up with the panel shut is only safe
//                   where renders cannot see it (see Na__North__CompassGizmo__). Instead the panel
//                   shuts itself, and the compass goes, when a Layout Editor sheet opens, and the
//                   compass steps out of the scene while any sheet picture is queued or rendering.
...
// - Ported from TrueVision3D's 1.1.0, without Show Compass; the panel shuts
//   when a sheet opens.
// - FOUND IN THE PORT'S OWN TEST: the compass was rendered into a sheet's 3D
//   viewport. The mode controller collapses every dev panel by class before
//   it announces a sheet, so a listener that first asked "is my panel open?"
//   always heard no, and left the compass in the scene. It no longer asks;
//   and the compass also leaves for the length of any snapshot render.
//
// =============================================================================
...
// -----------------------------------------------------------------------------
    ...
    import { Na__DrawData__CHANGED_EVENT, Na__DrawData__GetElevationsArray } from '../42__System__DrawingViewCore/Na__DrawView__ProjectData__.js';
    ...
    import { Na__NorthGizmo__RadiusFor, Na__NorthGizmo__Show, Na__NorthGizmo__Dispose } from './Na__North__CompassGizmo__.js';
...
// -----------------------------------------------------------------------------
    ...
    let Na__NorthDev__SheetBusy   = false;       // <-- A sheet picture is queued or rendering: the compass stays out of the scene until it is done
    // ------------------------------------------------------------
...
// -----------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    // The compass is up while the panel is open, and at no other time - and
    // not even then while a sheet picture is being rendered, because this app
    // has nothing that hides an authoring aid from a render.
    //
    ...
    function Na__NorthDev__SyncCompass() {
        ...
        const wanted  = Na__NorthDev__IsOpen() && !Na__NorthDev__SheetBusy;
    ...
    async function Na__North__DevMenu__Initialize(context) {
        ...
        toggle.addEventListener('click', () => {
            ...
            if (!isOpen) {
                ...
                // Authoring is done; the compass goes with the panel.
                Na__NorthPick__Cancel();
                Na__NorthDev__SyncCompass();
            }
        });
        ...
        // A SHEET OPENS | The compass must not be in the scene when a sheet
        // renders its drawings, so the panel shuts itself. The event is the
        // Layout Editor's own, heard by name: the editor loads lazily here and
        // must not be imported for the sake of one string.
        //
        // NOT ASKED WHETHER THE PANEL IS OPEN. The mode controller collapses
        // every dev panel by class before it announces, so by now this one
        // always reads as shut - and asking first left the compass standing in
        // the scene, where the sheet's 3D viewport rendered it into a drawing.
        // The announcement is made in the same turn the sheet is set, and a
        // render is queued behind a promise, so taking the compass out here is
        // always ahead of the first render.
        window.addEventListener('na-layouteditor-mode-changed', (event) => {
            if (!(event && event.detail && event.detail.isActive)) return;
        ...
        // ANY OTHER SHEET PICTURE | A sheet's viewports are also re-rendered
        // from the 3D tab - a scene saved in the Presentation editor restamps
        // every viewport that shows it - and this panel may be open when that
        // happens. The snapshot renderer announces its queue the moment a
        // render is booked, ahead of the render itself, so the compass is out
        // of the scene before the first one runs and back when the last is
        // done. Heard by name, as above. A compass being aimed is put down:
        // nobody is mid-click while a scene saves.
        window.addEventListener('na-layouteditor-snapshot-queue', (event) => {
            const busy = !!(event && event.detail && event.detail.outstanding > 0);
            if (busy === Na__NorthDev__SheetBusy) return;
            Na__NorthDev__SheetBusy = busy;
            if (busy) Na__NorthPick__Cancel();
            if (Na__NorthDev__IsOpen()) Na__NorthDev__Render();                  // <-- Draw Compass reads as it should again after a cancelled aim
            Na__NorthDev__SyncCompass();
        });
        return true;
    }
