...
//
// PORT NOTE:
// - Ported from   : ValeVision3D 51 Na__LayoutEditor__ViewportHandles__ (pattern)
...
//
// DEVELOPMENT LOG:
// 20-Sep-2026 - Version 1.8.0
// - RegisterGroupProvider, ported from TrueVision3D's Grips 1.8.0: a feature can
//   draw grips of its own on a selected group, after the group's box. The
//   Parametric Scrapbook's stretch and lookup grips, and its link noodle, arrive
//   this way.
//
...
// - Ported from TrueVision3D v2.52.0 (Grips 1.6.0).
//
// 14-Sep-2026 - Version 1.6.0
// - A selected group draws the blue box and "Group" overlay
//   (Na__LayoutEditor__Groups__). Vertex grips stay off.
// - Ported from TrueVision3D (Grips group render).
//
...
// - Ported from TrueVision3D (Grips 1.4.0).
//
...
// - Ported from TrueVision3D v2.45.0.
//
...
//   edge counter-scaled like the grips. Ported from TrueVision3D.
//
...
// -----------------------------------------------------------------------------
    ...
    // MODULE VARIABLES | The Rubber Band, the Rubber Box and the Insert Preview
    // ------------------------------------------------------------
   ...
   function Na__LeGrips__Add(layer, xMm, yMm, ppm, sizePx, zoom, modifier, picked) {
    ...
    function Na__LeGrips__Render(layer, sheet, selection, ppm, zoom) {
        ...
        if (selection.kind === 'shape') {
            ...
            // A picked vertex is drawn red, so a box selection of several reads
            // at a glance.
            // ------------------------------------
        ...
        if (selection.kind === 'group') {
            ...
            Na__LeGrips__GroupProviders.forEach((provider) => {                  // <-- A parametric element's own grips, over the group's box
                try { provider(layer, sheet, selection, ppm, zoom); } catch (error) { console.warn('[ValeVision3D LayoutEditor] A group grip provider failed.', error); }
            });
            return drawn;
        }
