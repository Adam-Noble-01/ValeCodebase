...
// -----------------------------------------------------------------------------
    ...
    //
    // The old `contextLayer` toggle - which hid the existing building and the
    // landscape categories - is gone from the panel. Two controls both claiming
    // to be the context, one of which emptied a renovation drawing entirely, was
    // worse than one that does the obvious thing. The key and its mechanism
    // remain in the record layer, defaulting on, so nothing saved changes
    // meaning and Na__LeSnap__HideContext still works if it is ever wanted back
