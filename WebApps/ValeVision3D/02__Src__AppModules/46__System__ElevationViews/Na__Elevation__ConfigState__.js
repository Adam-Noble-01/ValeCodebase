// =============================================================================
// VALEVISION3D - ELEVATION VIEWS - CONFIG STATE
// =============================================================================
//
...
//
// PORT NOTE:
// - Ported from   : TrueVision3D 02__Src__AppModules/45__System__ElevationViews/Na__Elevation__ConfigState__.js
...
//   - Import paths follow the ValeVision folder numbers (42 DrawingViewCore, 46 ElevationViews).
//   - Adds GetSectionGroupTarget (D28), GetFacePickSetup and GetGripSetup (D16) for the ValeVision additions.
// - Back-port     : none pending.
//
// -----------------------------------------------------------------------------
//
// DEVELOPMENT LOG:
// 09-Sep-2026 - ValeVision3D v2.19.0 (port Phase 3)
// - Ported from TrueVision3D for port Phase 3; logic unchanged, imports repointed.
//
...
// -----------------------------------------------------------------------------
    ...
    async function Na__ElevCfg__Fetch() {
        try {
            ...
            if (!response.ok) {
                console.warn('[ValeVision3D] Elevation config fetch failed (' + response.status + ') - using built-in defaults.');
                return false;
            }
            Na__ElevCfg__Config = await response.json();
            return Na__ElevCfg__IsEnabled();
        } catch (error) {
            console.warn('[ValeVision3D] Elevation config unreadable - using built-in defaults.', error);
            return false;
        }
    }
    // ------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// REGION | ValeVision Additions - Section Group, Face Pick and Grip (port Phase 3)
// -----------------------------------------------------------------------------

    // FUNCTION | Get the Target Scene Group for Section-Mode Drawings (D28)
    // ------------------------------------------------------------
    // A section files into Cross Sections rather than Elevations, so the two
    // groups always hold what their names say.
    // ------------------------------------------------------------
    function Na__ElevCfg__GetSectionGroupTarget() {
        return {
            groupName  : Na__ElevCfg__Val(Na__ElevCfg__GROUP_BLOCK, 'ElevationViews__SceneGroup__SectionTargetGroupName', 'Cross Sections'),
            groupId    : Na__ElevCfg__Val(Na__ElevCfg__GROUP_BLOCK, 'ElevationViews__SceneGroup__SectionTargetGroupId',   'Group_006'),
            autoEnable : Na__ElevCfg__Val(Na__ElevCfg__GROUP_BLOCK, 'ElevationViews__SceneGroup__AutoEnableTargetGroup', true) !== false
        };
    }
    // ------------------------------------------------------------
