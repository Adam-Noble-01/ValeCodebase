// =============================================================================
// VALEVISION3D - PRESENTATION MODE - SCENE GROUP SELECTOR UI
// =============================================================================
//
...
//   shows and hides with the carousel (the Views button, the Video Studio
//   timeline taking the bottom of the screen, and the scenes-cleared path all
//   act on the container) and needs no visibility logic of its own. It is
//   absolutely positioned so it overhangs the carousel's top edge without
//   affecting the strip's layout.
//
// INTEGRATION:
// - Initialized from index.html directly after the carousel, and awaited so
//   the group config is loaded before the project's scenes arrive.
...
//
// PORT NOTE:
// - Ported from   : TrueVision3D 02__Src__AppModules/21__System__PresentationMode/Na__PresentationMode__UI__SceneGroupSelector__.js
// - Source version: 1.0.0 (31-Aug-2026)
// - Ported on     : 09-Sep-2026 for ValeVision3D v2.17.0
// - Parity        : verbatim
// - Divergences   :
//   - Console prefix reads [ValeVision3D].
//   - The ValeVision carousel has no idle opacity fade, so the inherited-fade rationale from
//     TrueVision does not apply here; mounting inside the carousel is kept for show/hide parity.
...
// -----------------------------------------------------------------------------
    ...
    async function Na__PresentationMode__UI__InitializeSceneGroupSelector() {
        ...
        if (!Na__PresentationMode__SceneGroups__IsEnabled()) {
            console.log('[ValeVision3D] Scene groups disabled - selector bar not mounted.');
        ...
        console.log('[ValeVision3D] Presentation Mode scene group selector initialized.');
    }
    // ------------------------------------------------------------
