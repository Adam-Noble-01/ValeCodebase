// =============================================================================
// VALEVISION3D - LAYOUT EDITOR - EDGE STYLES
// =============================================================================
//
        ...
        if (!Na__LeEdge__LoadPromise) {
            Na__LeEdge__LoadPromise = (async () => {
                ...
                try {
                    ...
                    if (!response.ok) {
                        console.warn('[ValeVision3D LayoutEditor] Edge style config fetch failed (' + response.status + ') - the built-in palette will be used.');
                        return null;
                    }
                    Na__LeEdge__Config = await response.json();
                    Na__LeEdge__BuildIndexes();
                } catch (error) {
                    console.warn('[ValeVision3D LayoutEditor] Edge style config unavailable - the built-in palette will be used.', error);
                }
